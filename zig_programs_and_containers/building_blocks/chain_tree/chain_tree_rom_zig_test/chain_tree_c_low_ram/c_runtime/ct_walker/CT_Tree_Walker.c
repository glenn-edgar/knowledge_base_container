 ...
 */
  ...
  */
 static CT_ReturnCode walk_recursive(
     CT_TreeWalker* walker,
     unsigned int node_id,
     unsigned int level
 ) {
     /* Check max level */
     if (level > walker->max_level) {
     ...
     /* Mark as visited */
     walker->flags[node_id] |= CT_FLAG_VISITED;
     ...
     if (ret == CT_SKIP_CHILDREN) {
         return CT_CONTINUE;  /* Skip children but continue */
     }
     
     /* Get children */
     unsigned int children[256];  /* Static buffer for children */
     ...
     for (unsigned int i = 0; i < num_children; i++) {
         ...
         CT_ReturnCode child_ret = walk_recursive(walker, children[i], level + 1);
         
         if (child_ret == CT_STOP_ALL) {
             walker->stop_all = true;
             return child_ret;
         }
         
         if (child_ret == CT_STOP_SIBLINGS) {
             break;  /* Stop processing remaining siblings */
         }
     }
 ...
 static CT_ReturnCode walk_iterative(
     ...
     while (stack_top >= 0) {
         ...
         stack_top++;
         stack[stack_top].node_id = children[current.child_index];
 ...
 CT_ReturnCode ct_walker_walk(
     ...
     switch (method) {
         case CT_RECURSIVE:
             ret = walk_recursive(walker, root_id, 0);
             break;
