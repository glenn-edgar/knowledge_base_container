...
const log = std.log.scoped(.mqtt_rpc);
...
pub const Config = struct {
    ...
    service_name: [:0]const u8 = "rpc_service",
    keepalive: u16 = 60,
...
pub const Server = struct {
    ...
    pub fn init(cfg: Config, allocator: std.mem.Allocator) Error!Self {
        ...
        self.client_id = if (cfg.client_id) |id|
            ...
            autoClientId(allocator, "rpc_server") catch return Error.OutOfMemory;

        self.request_topic = std.fmt.allocPrintZ(allocator, "rpc/{s}/request/+", .{cfg.service_name}) catch return Error.OutOfMemory;
        self.response_topic_base = std.fmt.allocPrintZ(allocator, "rpc/{s}/response", .{cfg.service_name}) catch return Error.OutOfMemory;

        const mosq = c.mosquitto_new(self.client_id.ptr, true, null);
        if (mosq == null) {
            log.err("[rpc_server] mosquitto_new failed", .{});
    ...
    pub fn register(self: *Self, name: [:0]const u8, handler: MethodFn, userdata: ?*anyopaque) void {
        ...
        log.info("[rpc_server] registered method: {s}", .{name});
    }
    ...
    pub fn start(self: *Self, wait_for_suback: bool, sub_timeout_ms: u64) Error!void {
        ...
        if (rc_conn != c.MOSQ_ERR_SUCCESS) {
            log.err("[rpc_server] connect: {s}", .{c.mosquitto_strerror(rc_conn)});
            return Error.ConnectFailed;
        }

        const rc_loop = c.mosquitto_loop_start(self.mosq);
        if (rc_loop != c.MOSQ_ERR_SUCCESS) {
            log.err("[rpc_server] loop_start: {s}", .{c.mosquitto_strerror(rc_loop)});
...

        {
            ...
            if (!self.connected) {
                log.err("[rpc_server] connect timeout", .{});
        ...
        if (wait_for_suback) {
            ...
            if (!self.subscribed) {
                log.warn("[rpc_server] warning: SUBACK not received yet", .{});
            }
        }

        log.info("[rpc_server] started (service={s})", .{self.cfg.service_name});
    }
    ...
    fn processRequest(self: *Self, topic: []const u8, payload: []const u8) void {
        ...
        const parsed = std.json.parseFromSlice(std.json.Value, arena_alloc, payload, .{}) catch {
            const err_resp = std.fmt.allocPrintZ(self.allocator, "{{\"jsonrpc\":\"2.0\",\"id\":null,\"error\":{{\"code\":{d},\"message\":\"Parse error\"}}}}", .{JSONRPC_PARSE_ERROR}) catch return;
        ...
        const version = jsonGetString(root, "jsonrpc");
        const method_name = jsonGetString(root, "method");

        if (version == null or !std.mem.eql(u8, version.?, "2.0") or method_name == null) {
            const err_resp = std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"id\":{s},\"error\":{{\"code\":{d},\"message\":\"Invalid Request\"}}}}", .{ id_json, JSONRPC_INVALID_REQUEST }) catch return;
            defer self.allocator.free(err_resp);
            self.publishResponse(caller_id, err_resp);
            return;
        }
        ...
        if (handler == null) {
            const err_resp = std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"id\":{s},\"error\":{{\"code\":{d},\"message\":\"Method '{s}' not found\"}}}}", .{ id_json, JSONRPC_METHOD_NOT_FOUND, method_name.? }) catch return;
            defer self.allocator.free(err_resp);
            self.publishResponse(caller_id, err_resp);
            return;
        }
        ...
        if (result_json) |result| {
            const resp = std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"id\":{s},\"result\":{s}}}", .{ id_json, result }) catch return;
            defer self.allocator.free(resp);
            self.publishResponse(caller_id, resp);
        } else {
            const err_resp = std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"id\":{s},\"error\":{{\"code\":{d},\"message\":\"Method returned null\"}}}}", .{ id_json, JSONRPC_INTERNAL_ERROR }) catch return;
            defer self.allocator.free(err_resp);
            self.publishResponse(caller_id, err_resp);
        }
    }
    ...
    fn srvOnConnect(_: ?*c.struct_mosquitto, userdata: ?*anyopaque, rc: c_int) callconv(.C) void {
        ...
        self.mutex.lock();
        self.connected = (rc == 0);
        log.info("[rpc_server] on_connect rc={d}", .{rc});
    ...
    fn srvOnSubscribe(_: ?*c.struct_mosquitto, userdata: ?*anyopaque, mid: c_int, _: c_int, _: ?*const c_int) callconv(.C) void {
        const self: *Self = @ptrCast(@alignCast(userdata));
        log.info("[rpc_server] on_subscribe mid={d}", .{mid});
        self.mutex.lock();
        defer self.mutex.unlock();
    ...
    fn srvOnMessage(_: ?*c.struct_mosquitto, userdata: ?*anyopaque, msg: ?*const c.struct_mosquitto_message) callconv(.C) void {
        ...
        const thread = std.Thread.spawn(.{}, processThread, .{ self, topic_copy, payload_copy }) catch {
            log.err("[rpc_server] failed to spawn request thread", .{});
            self.allocator.free(topic_copy);
...
pub const Client = struct {
    ...
    pub fn init(cfg: Config, allocator: std.mem.Allocator, default_timeout_ms: u64) Error!Self {
        ...
        self.client_id = if (cfg.client_id) |id|
            ...
            autoClientId(allocator, "rpc_client") catch return Error.OutOfMemory;

        self.request_topic = std.fmt.allocPrintZ(allocator, "rpc/{s}/request/{s}", .{ cfg.service_name, self.client_id }) catch return Error.OutOfMemory;
        self.response_topic = std.fmt.allocPrintZ(allocator, "rpc/{s}/response/{s}", .{ cfg.service_name, self.client_id }) catch return Error.OutOfMemory;

        const mosq = c.mosquitto_new(self.client_id.ptr, true, null);
        if (mosq == null) {
            log.err("[rpc_client] mosquitto_new failed", .{});
            return Error.MosquittoNew;
        }
        self.mosq = mosq.?;
    ...
    pub fn connect(self: *Self, timeout_ms: u64) Error!void {
        ...
        if (rc_conn != c.MOSQ_ERR_SUCCESS) {
            log.err("[rpc_client] connect: {s}", .{c.mosquitto_strerror(rc_conn)});
            return Error.ConnectFailed;
        }

        const rc_loop = c.mosquitto_loop_start(self.mosq);
        if (rc_loop != c.MOSQ_ERR_SUCCESS) {
            log.err("[rpc_client] loop_start: {s}", .{c.mosquitto_strerror(rc_loop)});
            return Error.LoopStartFailed;
        }

        {
            ...
            if (!self.connected) {
                log.err("[rpc_client] connect timeout", .{});
...

        {
            ...
            if (!self.subscribed) {
                log.warn("[rpc_client] warning: SUBACK not received yet", .{});
            }
        }
    }

    pub fn disconnect(self: *Self) void {
        log.info("[rpc_client] disconnecting...", .{});
        _ = c.mosquitto_disconnect(self.mosq);
        _ = c.mosquitto_loop_stop(self.mosq, false);
        self.mutex.lock();
        defer self.mutex.unlock();
    ...
    pub fn call(
        ...
        if (!self.connected) {
            log.err("[rpc_client] call: not connected", .{});
        ...
        const payload = if (params_json) |params|
            std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"method\":\"{s}\",\"id\":\"{s}\",\"params\":{s}}}", .{ method, req_id, params }) catch return Error.OutOfMemory
        else
            std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"method\":\"{s}\",\"id\":\"{s}\"}}", .{ method, req_id }) catch return Error.OutOfMemory;
        defer self.allocator.free(payload);
        ...
        if (rc != c.MOSQ_ERR_SUCCESS) {
            log.err("[rpc_client] publish failed: {s}", .{c.mosquitto_strerror(rc)});
        ...
        if (!pend.done) {
            log.err("[rpc_client] call '{s}' timed out", .{method});
            self.allocator.free(req_id);
    ...
    fn parseResponse(self: *Self, response_json: []const u8) Error!CallResult {
        ...
        const parsed = std.json.parseFromSlice(std.json.Value, arena_alloc, response_json, .{}) catch return Error.RpcError;
    ...
    fn cliOnConnect(_: ?*c.struct_mosquitto, userdata: ?*anyopaque, rc: c_int) callconv(.C) void {
        ...
        self.mutex.lock();
        self.connected = (rc == 0);
        log.info("[rpc_client] on_connect rc={d}", .{rc});
    ...
    fn cliOnSubscribe(_: ?*c.struct_mosquitto, userdata: ?*anyopaque, mid: c_int, _: c_int, _: ?*const c_int) callconv(.C) void {
        const self: *Self = @ptrCast(@alignCast(userdata));
        log.info("[rpc_client] on_subscribe mid={d}", .{mid});
        self.mutex.lock();
        defer self.mutex.unlock();
        self.subscribed = true;
        self.subscribe_cond.signal();
...
test "Config defaults" {
    ...
    try std.testing.expectEqualStrings("rpc_service", cfg.service_name);
    try std.testing.expectEqual(@as(c_int, 1), cfg.qos);
}

test "extractCallerId" {
    try std.testing.expectEqualStrings("my-client", extractCallerId("rpc/svc/request/my-client").?);
    try std.testing.expectEqualStrings("abc_123", extractCallerId("rpc/test/request/abc_123").?);
    try std.testing.expect(extractCallerId("rpc/svc/request/") == null);
    try std.testing.expect(extractCallerId("rpc/svc") == null);
}
