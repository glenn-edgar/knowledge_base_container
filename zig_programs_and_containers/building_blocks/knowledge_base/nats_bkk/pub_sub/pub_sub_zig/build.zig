...
pub fn build(b: *std.Build) void {
    ...
    const c_lib_path: []const u8 = b.option(
        []const u8,
        "c-lib",
        "Path to pub_sub_c/build (compiled .so/.a files)",
    ) orelse "../pub_sub_c/build";

    // Library module
    const lib_mod = b.addModule("pub_sub_zig", .{
    ...
    integration.root_module.addImport("pub_sub_zig", lib_mod);
    ...
    example.root_module.addImport("pub_sub_zig", lib_mod);
...
fn linkCLibs(step: *std.Build.Step.Compile, c_lib_path: []const u8) void {
    ...
    step.linkSystemLibrary("nats_pubsub");
