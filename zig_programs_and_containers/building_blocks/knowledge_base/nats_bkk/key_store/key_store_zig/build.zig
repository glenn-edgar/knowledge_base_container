...
pub fn build(b: *std.Build) void {
    ...
    const c_lib_path: []const u8 = b.option(
        []const u8,
        "c-lib",
        "Path to key_store_c/build (compiled .so/.a files)",
    ) orelse "../key_store_c/build";

    // ----------------------------------------------------------------
    //  Library module – importable by test/example and downstream projects.
    //  No @cImport, so no C include paths required.
    // ----------------------------------------------------------------
    const lib_mod = b.addModule("key_store_zig", .{
    ...
    integration.root_module.addImport("key_store_zig", lib_mod);
    ...
    example.root_module.addImport("key_store_zig", lib_mod);
...
fn linkCLibs(step: *std.Build.Step.Compile, c_lib_path: []const u8) void {
    ...
    step.linkSystemLibrary("nats_key_store");
    step.linkSystemLibrary("nats_kb_store");
    step.linkSystemLibrary("nats_job_queue");
