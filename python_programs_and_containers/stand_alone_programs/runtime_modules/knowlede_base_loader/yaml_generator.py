...
class YamlGenerator:
    def __init__(self, yaml_file: Path, path_list: list = None):
        self.yaml_file = yaml_file
        self.path_list = path_list if path_list is not None else []
        self.ref_path = self.path_list.copy()
        ...
        # check path list for valid list
        if not isinstance(self.path_list, list):
            raise ValueError("Path list must be a list")
    ...
    def generate_yaml(self):
        """Generate/save the YAML file with ltree structure"""
        if self.path_list != self.ref_path:
            raise ValueError(f"Path list does not match: {self.path_list}")
        
        # Write yaml file with ltree structure
        with open(self.yaml_file, 'w') as f:
            yaml.dump(self.yaml_data, f, default_flow_style=False, sort_keys=True)
    ...
    def define_composite_node(self, label_name: str, node_name: str, 
        ...
        ltree_name = self._create_ltree_name(label_name, node_name)
...
if __name__ == "__main__":
    ...
    generator = YamlGenerator(yaml_file)
