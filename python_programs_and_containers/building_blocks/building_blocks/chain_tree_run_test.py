from chain_tree.ct_run.chain_tree_run import ChainTreeRun
...
def finalize_template_results(handle,node):
    ...
    print("template_mailbox",node["node_dict"]["template_mailbox"])
    for mailbox_name in node["node_dict"]["template_mailbox"]:
        template_name = node["node_dict"]["shorted_name_dict"][mailbox_name]
        output_data = handle["chain_tree"].template_functions.get_template_finalize_data(mailbox_name)
        if output_data is None:
            print("output_data is None for mailbox for template",template_name,"mapped to",mailbox_name)
        else:
            print("template output_data for template",template_name,"mapped to",mailbox_name,output_data)
...
if __name__ == "__main__":
    system_kb = "kb.test_subsystem"
    root_node_id = ChainTreeRun.get_root_node_id(system_kb)
    wait_seconds = .25
    ct = ChainTreeRun(wait_seconds=wait_seconds)
    ct.load_chain_tree(yaml_file=Path("basic_tests.yaml"))
    
    
    ct.Vo.add_one_shot_function_python("LOAD_TEMPLATE_DATA",load_template_data,description="Loads the template data")
    ct.Vo.add_one_shot_function_python("WAIT_FOR_EVENT_ERROR",wait_for_event_error,description="Waits for an event error")
    
    ct.Vo.add_one_shot_function_python("ACTIVATE_VALVE",activate_valve,description="Activates a valve")
    
    ct.Vo.add_one_shot_function_python("VERIFY_ERROR",verify_error,description="Verifies an error")
    
    ct.Vo.add_one_shot_function_python("DISPLAY_SEQUENCE_RESULT",display_sequence_result,description="Displays the sequence result")
    
    ct.Vb.add_boolean_function_python("WHILE_TEST",while_test,description="While test")
    
    ct.Vo.add_one_shot_function_python("WATCH_DOG_TIME_OUT",watch_dog_time_out,description="Watch dog time out")
    
    ct.Vo.add_one_shot_function_python("DISPLAY_FAILURE_WINDOW_RESULT",display_failure_window_result,
                                       description="Displays the failure window result")
    ct.Vb.add_boolean_function_python("DF_EXPRESSION",df_expression,description="Data flow expression")
    
    ct.Vb.add_boolean_function_python("DF_MASK",df_mask,description="Data flow mask")
    
    ct.Vo.add_one_shot_function_python("SET_TEMPLATE_OUTPUT_DATA",set_template_output_data,description="Sets the template output data")
    
    ct.Vo.add_one_shot_function_python("FINALIZE_TEMPLATE_RESULTS",finalize_template_results,description="Finalizes the template results")
    
    ct.Vo.add_one_shot_function_python("MY_EXCEPTION_LOGGING",my_exception_logging,description="Exception logging")
    
    ct.Vb.add_boolean_function_python("MY_EXCEPTION_DISPATCHER",my_exception_dispatcher,description="My exception dispatcher")
    ct.Vb.add_boolean_function_python("MY_TOP_EXCEPTION_DISPATCHER",my_top_exception_dispatcher,description="My top exception dispatcher")
    ...
    single_test = "nineteenth_test"
    single_test_flag = True
    if single_test_flag:
        print(f"*******************************************Running test: {single_test}*******************************************")
        ct.run(starting_kb=single_test)
        exit()

    print("test_list",test_list)
    for test in test_list:
        print(f"*******************************************Running test: {test}*******************************************")
        ct.run(starting_kb=test)