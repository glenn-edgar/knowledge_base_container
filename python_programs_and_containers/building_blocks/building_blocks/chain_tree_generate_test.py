...
def first_test(ct,ds,kb_name):
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
def second_test(ct,ds,kb_name):
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    
    activate_valve_column = ct.define_column(column_name="activate_valve", column_data=None,auto_start=False)
    ct.asm_one_shot_handler(one_shot_fn="ACTIVATE_VALVE",one_shot_data={"state":"open"})
    ct.asm_log_message("Valve activated")
    ct.asm_terminate()
    ct.end_column(column_name=activate_valve_column)
...
def third_test(ct,ds,kb_name):
    ds.add_kb(kb_name)#subscribed events
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    ...
    ct.asm_unsubscribe_events(node_id=subscribe_event,events=["PUBLISH_EVENT"])
    ct.asm_log_message("waiting 5 seconds")
    ct.asm_wait_time(time_delay=5)
    ct.asm_log_message("ending test")
    ct.asm_terminate_system()
...
def fourth_test(ct,ds,kb_name):
    ds.add_kb(kb_name)# nested column
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
def fifth_test(ct,ds,kb_name): # state machine
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    
    sm_name = "state_machine"
    state_machine = ct.define_state_machine(column_name="state_machine",sm_name=sm_name,state_names=["state1","state2","state3"],
                                            initial_state="state1",auto_start=True)
    
    state1 = ct.define_state(state_name="state1",column_data=None)
    ct.asm_log_message("state1")
    ct.asm_wait_time(time_delay=2)
    ct.asm_log_message("changing state to state2")
    ct.change_state(sm_node_id=state_machine,new_state="state2")
    ct.asm_halt()
    ct.end_column(column_name=state1)
    
    state2 = ct.define_state(state_name="state2",column_data=None)
    ct.asm_log_message("state2")
    ct.asm_wait_time(time_delay=2)
    ct.asm_log_message("changing state to state3")
    ct.change_state(state_machine,new_state="state3")
    ct.asm_halt()
    ct.end_column(column_name=state2)

    state3 = ct.define_state(state_name="state3",column_data=None)
    ct.asm_log_message("state3")
    ct.asm_wait_time(time_delay=2)
    ct.asm_log_message("changing state to state1")
    ct.change_state(state_machine,new_state="state1")
    ct.asm_halt()
    ct.end_column(column_name = state3)
    
    ct.end_state_machine(state_node=state_machine,sm_name="state_machine")
    
    column_id =ct.define_column(column_name="end_column", column_data=None,auto_start=True)
    ct.asm_wait_time(time_delay=20)
    ct.asm_log_message("ending test")
    ct.asm_terminate_system()
    ct.end_column(column_name=column_id)
    
    ct.finalize_and_check()
    

    
def insert_fork_column(ct,ds):
...
def insert_fork_join_column(ct,ds):
...
def seventh_test(ct,ds,kb_name): # fork column
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
def insert_fork_join_column_a(ct,ds):
...
def eighth_test(ct,ds,kb_name): # sequence til
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    ...
    insert_fork_join_column_a(ct,ds)
...
def insert_sequence_til_fail_column(ct,ds):
...
def ninth_test(ct,ds,kb_name): # sequence til
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    ...
    main_node = ct.define_sequence_start_node(column_name="main_node",finalize_function="DISPLAY_SEQUENCE_RESULT",auto_start=True)
    ct.asm_log_message("main node")
    insert_sequence_til_fail_column(ct,ds)
    ct.asm_log_message("main node is terminating")
    ct.asm_terminate()
    ct.end_column(column_name=main_node)
...
def test_one_for_one_test(ct,ds,top_column_name):
...
def test_one_for_all_test(ct,ds,top_column_name):
    top_column = ct.define_column(column_name=top_column_name,auto_start=True)
...
def test_rest_for_all_test(ct,ds,top_column_name):
...
def test_failure_window_test(ct,ds,top_column_name):
    top_column = ct.define_column(column_name=top_column_name,auto_start=True)
...
def tenth_test(ct,ds,kb_name): # supervisor node
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    ...
    test_one_for_one = test_one_for_one_test(ct,ds,"one_for_one_column")
    ct.define_join_link(test_one_for_one)
    
    
    ct.asm_log_message("starting test one for all")
    test_one_for_all = test_one_for_all_test(ct,ds,"one_for_all_column")
    ct.define_join_link(test_one_for_all)
    
    ct.asm_log_message("starting test rest for all")
    test_reset_for_all = test_rest_for_all_test(ct,ds,"rest_for_all_column")
    ct.define_join_link(test_reset_for_all)
    
    ct.asm_log_message("testing failure window test")
    test_failure_window = test_failure_window_test(ct,ds,"failure_window_column")
...
def eleventh_test(ct,ds,kb_name): # supervisor node
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
def twelfth_test(ct,ds,kb_name): # while column
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
def thirteenth_test(ct,ds,kb_name): # watch dog
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
def insert_register_event_column(ct,ds):
...
def insert_event_generator_column(ct,ds):
...
def insert_event_mask_df(ct,ds):
...
def insert_data_flow_expression_column(ct,ds):
...
def fourteenth_test(ct,ds,kb_name): # data flow
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    ...
    register_event_column = insert_register_event_column(ct,ds)
    ct.define_join_link(register_event_column)
   
    #### periodic data flow event set and clear
    event_generator = insert_event_generator_column(ct,ds)

    #### data flow event expression
    data_flow_expression_column = insert_data_flow_expression_column(ct,ds)
    #### data flow event mask
    data_flow_mask_column = insert_event_mask_df(ct,ds)
...
def fifteenth_test(ct,ds,kb_name): # templates
    define_template_a(ct,ds)
    define_template_b(ct,ds)
    define_template_c(ct,ds)
    define_template_d(ct,ds)
...
def sixteenth_test(ct,ds,kb_name): # templates
    define_template_e(ct,ds)
    define_template_f(ct,ds)
    define_template_g(ct,ds)
    define_template_h(ct,ds)
...
def test_unhandled_exception(ct,ds):
...
def seventeenth_test(ct,ds,kb_name): # exception handler
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
    ...
    if unhandled_test:
        test_unhandled_exception(ct,ds)
...
def eighteenth_test(ct,ds,kb_name): # exception handler
...
test_1 = "(pipeline (@CFL_LOGM__1 test_message1) (@CFL_LOGM__2 test_message2) 'CFL_DISABLE))" 
test_2 = "(pipeline (@CFL_LOGM__1 wait_for_three_seconds ) (!CFL_WAIT__1 3) (@CFL_LOGM__2 wait_for_two_seconds) (!CFL_WAIT__2 2) \
    (@CFL_LOGM__3 terminate_sequence) 'CFL_DISABLE))"
test_3 = "(pipeline (!CFL_TIME_OUT__1 10) (@CFL_LOGM__1 wait_for_three_seconds ) (!CFL_WAIT__1 3) (@CFL_LOGM__2 wait_for_two_seconds) (!CFL_WAIT__2 2) \
    (@CFL_LOGM__3 terminate_sequence) (@CFL_LOGM__4 wait_five_seconds_for_timeout) 'CFL_HALT))"
    




def nineteenth_test(ct,ds,kb_name): # s node control
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
link_test_1 = "(pipeline (@CFL_KILL_CHILDREN__0) (@CFL_FORK__0 0)  (@CFL_FORK___1 1) (!CFL_JOIN__1 0) (@CFL_FORK___2 2 ) (!CFL_JOIN___3 2)\
    (!CFL_JOIN__4 0) \
                                                                                                           'CFL_TERMINATE_FUNCTION))" 
link_test_2 = "(pipeline (@CFL_KILL_CHILDREN__0) (@CFL_FORK___0 0) (!CFL_JOIN__0 0) (@CFL_FORK___1 1) (!CFL_JOIN__1 1)\
                        (@CFL_FORK__2 2) (!CFL_JOIN__2 2) 'CFL_TERMINATE_FUNCTION))" 
link_test_3 = "(pipeline (@CFL_KILL_CHILDREN__0) (@CFL_FORK__0 0) (@CFL_FORK__1 1) (!CFL_JOIN__1 0) (!CFL_WAIT__0 4) (@CFL_TERMINATE__1 1) \
             (@CFL_FORK__2 2) (!CFL_JOIN__2 2) 'CFL_TERMINATE_FUNCTION))"

 
def twentieth_test(ct,ds,kb_name): # s node control
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
link_test_4 = """
  (dispatch event_id
  ("CFL_INIT_EVENT" (pipeline (@CFL_SET_INITIAL_STATE__1 0)  !RESET_CODES 'CFL_FUNCTION_RETURN))
  ("CFL_TERM_EVENT" (pipeline (@CFL_KILL_CHILDREN__0) 'CFL_FUNCTION_RETURN))
 
  ("CFL_CHANGE_STATE" (pipeline (@CFL_SET_STATE__1 0)  !RESET_CODES 'CFL_FUNCTION_HALT))
...
def twenty_first_test(ct,ds,kb_name): # s node control
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
link_test_5 = """
  (dispatch event_id
  ("CFL_INIT_EVENT" (pipeline (@CFL_KILL_CHILDREN__0) (@CFL_FORK__0 0) (@CFL_FORK__1 1) !RESET_CODES 'CFL_FUNCTION_RETURN))
  ("CFL_TERM_EVENT" (pipeline (@CFL_KILL_CHILDREN__0) !RESET_CODES 'CFL_FUNCTION_RETURN))
 
  ("CFL_CHANGE_STATE" (cond  ((?CFL_IS_STATE__0 0) (pipeline (@CFL_KILL_CHILDREN__0)(@CFL_FORK__0 0)(@CFL_FORK__1 1) !RESET_CODES 'CFL_FUNCTION_RETURN) )
                             ((?CFL_IS_STATE__1 1) (pipeline (@CFL_KILL_CHILDREN__0)(@CFL_FORK__2 2)(@CFL_FORK__3 3) !RESET_CODES 'CFL_FUNCTION_RETURN) )
                             (else (pipeline (@CFL_LOGM__0 "Invalid state") !RESET_CODES 'CFL_FUNCTION_TERMINATE))))
...
def twenty_second_test(ct,ds,kb_name): # s node control
    ds.add_kb(kb_name)
    ds.select_kb(kb_name)
    ct.define_root_node(version="1.0.0")
...
def add_ending(ct,ds):
    ct.finalize_template_kb()
    ct.write_yaml()
    ds.generate_yaml()
...
if __name__ == "__main__":
    ...
    single_test = "twentieth_test"
    single_test_flag = False
    if single_test_flag:
        ct,ds = add_header("basic_tests.yaml")
        test_dict[single_test](ct,ds,single_test)
        add_ending(ct,ds)
        exit()
