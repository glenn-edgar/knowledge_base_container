...
class KB_Search:
    ...
    def __init__(self, host, port, dbname, user, password, database):
        ...
        self._initialize_common_attributes(database)
        
        # Mark as internal connection
    ...
    def from_connection(cls, connection: psycopg2.extensions.connection, database: str):
        """
        Create KB_Search instance from an existing psycopg2 connection.
        
        Args:
            connection: Existing psycopg2 connection object
            database: The name of the table to query
            
        Returns:
            KB_Search instance using the provided connection
        """
        # Create instance without calling __init__
        instance = cls.__new__(cls)
        
        # Initialize common attributes
        instance._initialize_common_attributes(database)
        
        # Set up connection-related attributes
        instance._setup_external_connection(connection)
        
        return instance
    
    def _initialize_common_attributes(self, database: str):
        """
        Initialize attributes common to both constructors.
    ...
    def _setup_external_connection(self, connection: psycopg2.extensions.connection):
        """
        Set up the instance to use an external connection.
        
        Args:
            connection: Existing psycopg2 connection object
        """
        # Set connection and mark as external
        self.conn = connection
        self._external_connection = True
        
        # Create a RealDictCursor for this instance
        self.cursor = connection.cursor(cursor_factory=RealDictCursor)
        
        # Extract connection parameters for reference
        self._extract_connection_params()
    
    def _extract_connection_params(self):
        """
        Extract connection parameters from an existing connection for reference.
        Sets host, port, dbname, user attributes based on the connection.
        """
        try:
            dsn_params = self.conn.get_dsn_parameters()
            self.host = dsn_params.get('host', 'unknown')
            self.port = dsn_params.get('port', 5432)
            self.dbname = dsn_params.get('dbname', 'unknown')
            self.user = dsn_params.get('user', 'unknown')
            self.password = None  # Password not retrievable from connection
        except:
