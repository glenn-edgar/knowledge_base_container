...
def cfl_change_state(handle,node):
    ...
    sm_node = chain_tree.python_dict[sm_node_id]
    if sm_node["ct_control"]["enabled"] == False:
        return # state machine is not enabled
...
def cfl_use_template_init(handle,node):
    ...
    node["node_dict"]["template_mailbox"] = []
    node["node_dict"]["shorted_name_dict"] = {}
    for items in template_list:
        template_name = items[0]
        initial_conditions = items[1]
        shorted_name = chain_tree.template_functions.install_template(node, template_name,initial_conditions)
...
def cfl_use_template_term(handle,node):
    ...
    finalize_function = node["node_dict"]["finalize_function_name"]
    chain_tree.Vo.run_one_shot_function(finalize_function,handle,node)
...
def cfl_s_node_control_init(handle,node):
    ...
    function_dispath_dict = node["node_dict"]["column_data"]["function_dispatch_dict"]
    for function_name, dispatch_dict in function_dispath_dict.items():
        if dispatch_dict["type"] == "link_fork" or dispatch_dict["type"] == "link_join":
            dispatch_dict["active"] = False
