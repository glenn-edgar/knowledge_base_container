...
class TemplateFunctions:
    def __init__(self,chain_tree,ct_engine,template_kb_name="kb.T"):
    ...
    def install_template(self,node,template_name,initial_conditions):
        ...
        template_root_node_id = self.template_kb_name 
        template_root_node = self.chain_tree.python_dict[template_root_node_id]
        node_dict = template_root_node["node_dict"]
        if template_name not in node_dict:
            raise Exception(f"Template {template_name} not found")
        template_start_node_id = node_dict[template_name]
        ...
        shorted_name= "kb." + shorted_name
        node["node_dict"]["template_mailbox"].append(shorted_name)
    
        new_root_node_id = shorted_name + "." + template_start_node_id
        self.instanciated_template_dict[instanciated_template_name] = new_root_node_id
        self.load_instanciated_template(template_start_node_id,shorted_name,new_root_node_id)
        node["label_dict"]["links"].append(new_root_node_id)
    ...
    def load_instanciated_template(self,template_start_node_id,shorted_name,new_root_node_id):
        ...
        user_handle = []
        ct_walker.walk(start_node=template_start_node_id,method='bfs',user_handle=user_handle)
        ...
        for node in user_handle:
            ...
            label_dict["ltree_name"] = shorted_name + "." + label_dict["ltree_name"]
            label_dict["parent_ltree_name"] = shorted_name + "." + label_dict["parent_ltree_name"]
            new_links = []
            if "links" in label_dict:
                for link in label_dict["links"]:
                    link = shorted_name + "." + link
    ...
    def get_template_input_data(self,node):
        ...
        shorted_name = shorted_list[0]+"."+shorted_list[1]
    ...
    def set_template_output_data(self,node,output_data):
        ...
        shorted_list = ltree_name.split(".")
        shorted_name = shorted_list[0]+"."+shorted_list[1]
        self.output_data_dict[shorted_name] = output_data
    ...
    def get_template_finalize_data(self,mailbox_name):
        if mailbox_name not in self.output_data_dict:
            return None
        return self.output_data_dict[mailbox_name]
