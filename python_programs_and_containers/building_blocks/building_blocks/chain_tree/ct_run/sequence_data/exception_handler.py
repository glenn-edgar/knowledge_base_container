...
class ExceptionCatchHandler:
    ...
    def build_tree(self,starting_node):
        ...
        self.exception_tree = self.filtered_tree_builder.construct_tree(starting_node)
    
        return self.exception_tree
        
    def check_node_exists(self,node):
        if node["label_dict"]["ltree_name"] not in self.exception_tree:
            raise ValueError(f"Node {node['node_id']} not found in exception tree")
        return True
    ...
    def find_nearest_exception_handler(self,node):
        ...
        while current is not None:
            if current in self.exception_tree:
                return current
