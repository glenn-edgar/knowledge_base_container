...
class CT_Timer(CT_Events):
    ...
    def __init__(self, wait_seconds, root_node_id):
    ...
    def timer_tick(self,node_id):
        ...
        self.add_event(node_id,"CFL_TIMER_EVENT",self.tick_dict)

        for key, value in result['changed'].items():
           
            if key == 'second':
                
                self.add_event(self.root_node_id,"CFL_SECOND_EVENT", current_info['second'])
            if key == 'minute':
                self.add_event(self.root_node_id,"CFL_MINUTE_EVENT", current_info['minute'])
            if key == 'hour':
                self.add_event(self.root_node_id,"CFL_HOUR_EVENT", current_info['hour'])
            if key == 'day':
                self.add_event(self.root_node_id,"CFL_DAY_EVENT", current_info['day'])
            if key == 'dow':
                self.add_event(self.root_node_id,"CFL_DOW_EVENT", current_info['dow'])
            if key == 'doy':
                self.add_event(self.root_node_id,"CFL_DOY_EVENT", current_info['doy'])
            if key == 'month':
                self.add_event(self.root_node_id,"CFL_MONTH_EVENT", current_info['month'])
            if key == 'year':
                self.add_event(self.root_node_id,"CFL_YEAR_EVENT", current_info['year'])
