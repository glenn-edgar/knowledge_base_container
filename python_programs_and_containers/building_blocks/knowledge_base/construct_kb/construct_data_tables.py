...
class Construct_Data_Tables:
    ...
    def __init__(self, host, port, dbname, user, password, data_table_name: str = "knowledge_base"):
        """
        ...
        self.kb = Construct_KB(host, port, dbname, user, password, data_table_name)
        
        # Initialize all other components
        self._initialize_tables(data_table_name)
        self._map_methods()
    
    @classmethod
    def from_connection(cls, connection: psycopg2.extensions.connection, data_table_name: str):
        """
        Create Construct_Data_Tables instance from an existing psycopg2 connection.
        
        Args:
            connection: Existing psycopg2 connection object
            database: base knowledge base table name
            
        Returns:
            Construct_Data_Tables instance using the provided connection
        """
        # Create instance without calling __init__
        instance = cls.__new__(cls)
        
        # Create KB using its from_connection method
        instance.kb = Construct_KB.from_connection(connection, data_table_name)
        
        # Initialize all other components
        instance._initialize_tables(data_table_name)
        instance._map_methods()
        
        return instance
    
    def _initialize_tables(self, data_table_name: str):
        """
        Initialize all table constructor instances.
        
        Args:
            database: base knowledge base table name
        """
        self.status_table = Construct_Status_Table(
            self.kb.conn, 
            self.kb.cursor, 
            construct_kb=self.kb, 
            database=data_table_name
        )
        
        self.job_table = Construct_Job_Table(
            self.kb.conn, 
            self.kb.cursor, 
            self.kb, 
            database=data_table_name
        ...
        self.stream_table = Construct_Stream_Table(
            self.kb.conn, 
            self.kb.cursor, 
            self.kb, 
            database=data_table_name
        ...
        self.rpc_client_table = Construct_RPC_Client_Table(
            self.kb.conn, 
            self.kb.cursor, 
            self.kb, 
            database=data_table_name
        ...
        self.rpc_server_table = Construct_RPC_Server_Table(
            self.kb.conn, 
            self.kb.cursor, 
            self.kb, 
            database=data_table_name
    ...
    def _map_methods(self):
        """
        Map methods from KB and table instances to this class for convenience.
        """
        # Map KB methods
