 ...
 */
     ...
     for (uint16_t i = 0; i < hdr->section_count; i++) {
         if (dir[i].section_type == section_type) {
             if (dir[i].offset + dir[i].size > image_size)
                 return CFL_IMAGE_LOADER_SECTION_OOB ;
 ...
 {
     if (!image_data || !out) return CFL_IMAGE_LOADER_NULL_PTR;
     if (image_size < sizeof(ctb_header_t)) return CFL_IMAGE_LOADER_TOO_SMALL;
     ...
     /* Validate magic */
     if (hdr->magic != CTB_MAGIC) return CFL_IMAGE_LOADER_BAD_MAGIC;
 
     /* Validate version */
     if (hdr->version_major != CTB_VERSION_MAJOR) return CFL_IMAGE_LOADER_BAD_VERSION;
 
     /* Validate size */
     if (hdr->total_image_size != image_size) return CFL_IMAGE_LOADER_BAD_SIZE;
     ...
     /* Validate checksum */
     {
         ...
         if (crc != stored_crc) return CFL_IMAGE_LOADER_BAD_CHECKSUM;
     }
     ...
     if (dir_end > image_size) return CFL_IMAGE_LOADER_TOO_SMALL;
     ...
     if (rc != CFL_IMAGE_LOADER_OK) return rc;
     ...
     if (sect.data && hdr->event_count > 0) {
         ...
         if (!event_ptrs) return CFL_IMAGE_LOADER_ALLOC;
     ...
     if (sect.data && hdr->bitmask_count > 0) {
         ...
         if (!bmask_ptrs) return CFL_IMAGE_LOADER_ALLOC;
     ...
     if (sect.data && hdr->kb_count > 0) {
         ...
         if (!kb_infos) return CFL_IMAGE_LOADER_ALLOC;
         ...
         for (uint16_t i = 0; i < hdr->kb_count; i++) {
             ...
             if (kb_entries[i].alias_count > 0 && alias_entries) {
                 ...
                 if (!aliases) { free(kb_infos); return CFL_IMAGE_LOADER_ALLOC; }
     ...
     /* Build function name string arrays for the handle */
     {
         ...
         if (!main_names || !os_names || !bool_names) return CFL_IMAGE_LOADER_ALLOC;
 ...
 {
     if (!img || !name || !fn) return CFL_IMAGE_LOADER_NULL_PTR;
     uint32_t hash = fnv1a_32(name);
     int slot = binary_search_u32(img->main_hashes, img->main_func_count, hash);
     if (slot < 0) return CFL_IMAGE_LOADER_HASH_NOT_FOUND;
 ...
 int cfl_image_loader_validate(const cfl_image_loader_t *img)
 {
     ...
     for (uint16_t i = 1; i < img->main_func_count; i++) {
         if (!img->main_fn_ptrs[i]) {
             const char *name = cfl_image_loader_get_func_name(img, CTB_SECT_MFHT, i);
     ...
     for (uint16_t i = 1; i < img->one_shot_func_count; i++) {
         if (!img->one_shot_fn_ptrs[i]) {
             const char *name = cfl_image_loader_get_func_name(img, CTB_SECT_OSHT, i);
     ...
     for (uint16_t i = 1; i < img->boolean_func_count; i++) {
         if (!img->boolean_fn_ptrs[i]) {
             const char *name = cfl_image_loader_get_func_name(img, CTB_SECT_BFHT, i);
