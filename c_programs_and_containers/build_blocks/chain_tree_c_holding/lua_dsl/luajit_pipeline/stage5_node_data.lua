...
local function is_json_null(val)
    return type(val) == "table" and tostring(val) == "null"
end

local function is_array(t)
    if type(t) ~= "table" then return false end
    if is_json_null(t) then return false end
    local count = 0
...
function JsonRecordEncoder:encode_value(value)
    ...
    elseif is_json_null(value) then
        self.records[#self.records + 1] = { 3, 0 }  -- NULL
...
local function has_meaningful_data(data)
    ...
    if is_json_null(data) then return true end
