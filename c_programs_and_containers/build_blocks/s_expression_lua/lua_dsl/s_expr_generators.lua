...
-- S-Expression Engine Code Generators - Version 5.4
-- 
-- Contains:
...
--
-- VERSION 5.4 CHANGES:
--   - Added to_c_lua_registration() for Lua trampoline wiring
--   - Added to_c_lua_registration_header() for Lua trampoline .h file
--   - Added to_c_lua_hashes_header() for Lua-side hash manifest
--   - to_c_user_header() and to_c_user_registration() now exclude
--     Lua-declared functions (they get trampolines instead of C prototypes)
--
-- VERSION 5.3 CHANGES:
...
-- ============================================================================
-- LUA FUNCTION SET BUILDER (shared by multiple generators)
-- ============================================================================

local function build_lua_set(mod)
    local lua_set = {}
    if mod.lua_oneshot_funcs then
        for _, f in ipairs(mod.lua_oneshot_funcs) do lua_set[f.name] = true end
    end
    if mod.lua_main_funcs then
        for _, f in ipairs(mod.lua_main_funcs) do lua_set[f.name] = true end
    end
    if mod.lua_pred_funcs then
        for _, f in ipairs(mod.lua_pred_funcs) do lua_set[f.name] = true end
    end
    return lua_set
end
...
function ModuleGenerator:to_c_records_header(base_name)
    ...
    table.insert(lines, "// DO NOT EDIT - Generated by s_expr_dsl v5.4")
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "")
...
function ModuleGenerator:to_c_user_header(base_name)
    ...
    table.insert(lines, "// NOTE: Lua functions are in " .. base_name .. "_lua_registration.c")
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "")
    table.insert(lines, "#ifndef " .. guard)
    table.insert(lines, "#define " .. guard)
    ...
    -- Build lua function lookup set
    local lua_set = build_lua_set(mod)
    
    -- Filter out builtin AND lua functions
    local user_oneshot = {}
    local user_main = {}
    local user_pred = {}
    
    for _, name in ipairs(mod.oneshot_funcs) do
        if not builtins_module.is_builtin(name) and not lua_set[name] then
    ...
    if #user_oneshot > 0 then
        table.insert(lines, "// Oneshot functions (C-native)")
    ...
    if #user_main > 0 then
        table.insert(lines, "// Main functions (C-native)")
    ...
    if #user_pred > 0 then
        table.insert(lines, "// Predicate functions (C-native)")
...
function ModuleGenerator:to_c_user_registration(base_name)
    ...
    table.insert(lines, "// NOTE: Lua functions are registered via " .. base_name .. "_register_lua()")
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "")
    ...
    if #user_oneshot > 0 then
        table.insert(lines, "// Oneshot function entries (C-native)")
    ...
    if #user_main > 0 then
        table.insert(lines, "// Main function entries (C-native)")
    ...
    if #user_pred > 0 then
        table.insert(lines, "// Predicate function entries (C-native)")
    ...
    -- Register-all function (C-native only)
    table.insert(lines, "// Register all C-native user functions with module")
...
-- ============================================================================
-- LUA TRAMPOLINE REGISTRATION GENERATOR
-- ============================================================================

function ModuleGenerator:to_c_lua_registration(base_name)
    local lines = {}
    local mod = self.module
    
    local lua_oneshot = mod.lua_oneshot_funcs or {}
    local lua_main = mod.lua_main_funcs or {}
    local lua_pred = mod.lua_pred_funcs or {}
    
    if #lua_oneshot == 0 and #lua_main == 0 and #lua_pred == 0 then
        return nil  -- Nothing to generate
    end
    
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "// " .. base_name .. "_lua_registration.c")
    table.insert(lines, "// Lua trampoline registration for " .. mod.name)
    table.insert(lines, "// DO NOT EDIT - Generated by s_expr_dsl v5.4")
    table.insert(lines, "//")
    table.insert(lines, "// All Lua functions route through type-specific trampolines.")
    table.insert(lines, "// The Lua runtime registry must have matching functions registered")
    table.insert(lines, "// by hash before tree execution.")
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "")
    table.insert(lines, '#include "' .. base_name .. '.h"')
    table.insert(lines, '#include "se_lua53_bridge.h"')
    table.insert(lines, "")
    
    -- Oneshot entries
    if #lua_oneshot > 0 then
        table.insert(lines, "// Lua oneshot function entries")
        table.insert(lines, "static s_expr_fn_entry_t " .. base_name .. "_lua_oneshot_entries[] = {")
        for _, f in ipairs(lua_oneshot) do
            table.insert(lines, string.format("    { %s, (void*)se_lua_oneshot_trampoline },  // %s",
                hash_module.fmt_hash(f.hash), f.name))
        end
        table.insert(lines, "};")
        table.insert(lines, "")
    end
    ...
    -- Main entries
    if #lua_main > 0 then
        table.insert(lines, "// Lua main function entries")
        table.insert(lines, "static s_expr_fn_entry_t " .. base_name .. "_lua_main_entries[] = {")
        for _, f in ipairs(lua_main) do
            table.insert(lines, string.format("    { %s, (void*)se_lua_main_trampoline },  // %s",
                hash_module.fmt_hash(f.hash), f.name))
        end
        table.insert(lines, "};")
        table.insert(lines, "")
    ...
    -- Pred entries
    if #lua_pred > 0 then
        table.insert(lines, "// Lua predicate function entries")
        table.insert(lines, "static s_expr_fn_entry_t " .. base_name .. "_lua_pred_entries[] = {")
        for _, f in ipairs(lua_pred) do
            table.insert(lines, string.format("    { %s, (void*)se_lua_pred_trampoline },  // %s",
                hash_module.fmt_hash(f.hash), f.name))
        end
        table.insert(lines, "};")
        table.insert(lines, "")
    ...
    -- Tables
    if #lua_oneshot > 0 then
        table.insert(lines, "static const s_expr_fn_table_t " .. base_name .. "_lua_oneshot_table = {")
        table.insert(lines, "    .entries = " .. base_name .. "_lua_oneshot_entries,")
        table.insert(lines, "    .count = " .. #lua_oneshot)
        table.insert(lines, "};")
        table.insert(lines, "")
    end
    
    if #lua_main > 0 then
        table.insert(lines, "static const s_expr_fn_table_t " .. base_name .. "_lua_main_table = {")
        table.insert(lines, "    .entries = " .. base_name .. "_lua_main_entries,")
        table.insert(lines, "    .count = " .. #lua_main)
        table.insert(lines, "};")
        table.insert(lines, "")
    end
    
    if #lua_pred > 0 then
        table.insert(lines, "static const s_expr_fn_table_t " .. base_name .. "_lua_pred_table = {")
        table.insert(lines, "    .entries = " .. base_name .. "_lua_pred_entries,")
        table.insert(lines, "    .count = " .. #lua_pred)
        table.insert(lines, "};")
        table.insert(lines, "")
    end
    
    -- Register function
    table.insert(lines, "// Register all Lua trampoline functions with module")
    table.insert(lines, "void " .. base_name .. "_register_lua(s_expr_module_t* module) {")
    if #lua_oneshot > 0 then
        table.insert(lines, "    s_expr_module_register_oneshot(module, &" .. base_name .. "_lua_oneshot_table);")
    end
    if #lua_main > 0 then
        table.insert(lines, "    s_expr_module_register_main(module, &" .. base_name .. "_lua_main_table);")
    end
    if #lua_pred > 0 then
        table.insert(lines, "    s_expr_module_register_pred(module, &" .. base_name .. "_lua_pred_table);")
    end
    table.insert(lines, "}")
...
-- ============================================================================
-- LUA TRAMPOLINE REGISTRATION HEADER GENERATOR
-- ============================================================================

function ModuleGenerator:to_c_lua_registration_header(base_name)
    local lines = {}
    local mod = self.module
    ...
    local guard = base_name:upper() .. "_LUA_REGISTRATION_H"
    
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "// " .. base_name .. "_lua_registration.h")
    table.insert(lines, "// Lua trampoline registration header for " .. mod.name)
    table.insert(lines, "// DO NOT EDIT - Generated by s_expr_dsl v5.4")
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "")
    table.insert(lines, "#ifndef " .. guard)
...
-- ============================================================================
-- LUA HASH MANIFEST HEADER GENERATOR
-- ============================================================================

function ModuleGenerator:to_c_lua_hashes_header(base_name)
    local lines = {}
    local mod = self.module
    ...
    local guard = base_name:upper() .. "_LUA_HASHES_H"
    
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "// " .. base_name .. "_lua_hashes.h")
    table.insert(lines, "// Hash manifest for Lua runtime function registration")
    table.insert(lines, "// DO NOT EDIT - Generated by s_expr_dsl v5.4")
    table.insert(lines, "//")
    table.insert(lines, "// The Lua runtime must register functions for each hash below")
    table.insert(lines, "// before executing trees that reference them.")
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "")
    table.insert(lines, "#ifndef " .. guard)
    table.insert(lines, "#define " .. guard)
    ...
    if #lua_oneshot > 0 then
        table.insert(lines, "// Lua oneshot functions")
        for _, f in ipairs(lua_oneshot) do
            local def = "LUA_" .. f.name:upper():gsub("[^%w]", "_") .. "_HASH"
            table.insert(lines, string.format("#define %s %s  // %s", def, hash_module.fmt_hash(f.hash), f.name))
        end
        table.insert(lines, "")
    end
    
    if #lua_main > 0 then
        table.insert(lines, "// Lua main functions")
        for _, f in ipairs(lua_main) do
            local def = "LUA_" .. f.name:upper():gsub("[^%w]", "_") .. "_HASH"
            table.insert(lines, string.format("#define %s %s  // %s", def, hash_module.fmt_hash(f.hash), f.name))
        end
    ...
    if #lua_pred > 0 then
        table.insert(lines, "// Lua predicate functions")
        for _, f in ipairs(lua_pred) do
            local def = "LUA_" .. f.name:upper():gsub("[^%w]", "_") .. "_HASH"
            table.insert(lines, string.format("#define %s %s  // %s", def, hash_module.fmt_hash(f.hash), f.name))
        end
...
function ModuleGenerator:dump()
    ...
    -- Build lua set for annotation
    local lua_set = build_lua_set(mod)
    
    table.insert(lines, "FUNCTIONS:")
    local function annotate_list(funcs)
        local parts = {}
        for _, name in ipairs(funcs) do
            if lua_set[name] then
                table.insert(parts, name .. " [lua]")
            elseif builtins_module.is_builtin(name) then
                table.insert(parts, name .. " [builtin]")
            else
    ...
    table.insert(lines, "  Oneshot (" .. #mod.oneshot_funcs .. "): " .. annotate_list(mod.oneshot_funcs))
    table.insert(lines, "  Main (" .. #mod.main_funcs .. "): " .. annotate_list(mod.main_funcs))
    table.insert(lines, "  Pred (" .. #mod.pred_funcs .. "): " .. annotate_list(mod.pred_funcs))
    
    -- Lua-specific summary
    local lua_oneshot = mod.lua_oneshot_funcs or {}
    local lua_main = mod.lua_main_funcs or {}
    local lua_pred = mod.lua_pred_funcs or {}
    local lua_total = #lua_oneshot + #lua_main + #lua_pred
    if lua_total > 0 then
        table.insert(lines, "")
        table.insert(lines, "LUA FUNCTIONS (" .. lua_total .. "):")
        if #lua_oneshot > 0 then
            local names = {}
            for _, f in ipairs(lua_oneshot) do table.insert(names, f.name) end
            table.insert(lines, "  Lua oneshot (" .. #lua_oneshot .. "): " .. table.concat(names, ", "))
        end
        if #lua_main > 0 then
            local names = {}
            for _, f in ipairs(lua_main) do table.insert(names, f.name) end
            table.insert(lines, "  Lua main (" .. #lua_main .. "): " .. table.concat(names, ", "))
        end
        if #lua_pred > 0 then
            local names = {}
            for _, f in ipairs(lua_pred) do table.insert(names, f.name) end
            table.insert(lines, "  Lua pred (" .. #lua_pred .. "): " .. table.concat(names, ", "))
        end
    end
...
local SEXB_VERSION = 0x0504      -- Version 5.4
...
function BinaryModuleGenerator:to_c_header(base_name)
    ...
    table.insert(lines, "// Version: 5.4 (direct s_expr_param_t, zero-copy, parent_offset)")
    table.insert(lines, "// DO NOT EDIT - Generated by s_expr_dsl v5.4")
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "")
    table.insert(lines, "#ifndef " .. guard)
