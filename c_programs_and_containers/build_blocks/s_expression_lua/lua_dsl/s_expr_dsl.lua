-- ============================================================================
-- s_expr_dsl.lua
-- S-Expression Engine DSL Core Library - Version 5.4
-- 
...
--
-- VERSION 5.4 CHANGES:
--   - Added lua_oneshot(), lua_main(), lua_pred() for Lua 5.3 function declarations
--   - Added LUA_FUNCTIONS{} batch declaration
--   - Lua functions get trampoline registration instead of C prototypes
--   - Generated lua_registration.c and lua_hashes.h outputs
--
-- VERSION 5.3 CHANGES:
...
function _G.lua_oneshot(func_name)
    if not current_module then dsl_error("No module started") end
    if type(func_name) ~= "string" then dsl_error("lua_oneshot: name must be a string") end
    
    -- Check for duplicates in lua list
    for _, f in ipairs(current_module.lua_oneshot_funcs) do
        if f.name == func_name then
            dsl_error("Lua oneshot function already declared: " .. func_name)
        end
    end
    
    table.insert(current_module.lua_oneshot_funcs, {
        name = func_name,
        hash = hash_module.fnv1a_32(func_name),
    })
    
    -- Also register in the normal oneshot list so binary/headers include it
    local found = false
    for _, n in ipairs(current_module.oneshot_funcs) do
        if n == func_name then found = true; break end
    end
    if not found then table.insert(current_module.oneshot_funcs, func_name) end
end

function _G.lua_main(func_name)
    if not current_module then dsl_error("No module started") end
    if type(func_name) ~= "string" then dsl_error("lua_main: name must be a string") end
    
    for _, f in ipairs(current_module.lua_main_funcs) do
        if f.name == func_name then
            dsl_error("Lua main function already declared: " .. func_name)
        end
    end
    
    table.insert(current_module.lua_main_funcs, {
        name = func_name,
        hash = hash_module.fnv1a_32(func_name),
    })
    
    local found = false
    for _, n in ipairs(current_module.main_funcs) do
        if n == func_name then found = true; break end
    end
    if not found then table.insert(current_module.main_funcs, func_name) end
end

function _G.lua_pred(func_name)
    if not current_module then dsl_error("No module started") end
    if type(func_name) ~= "string" then dsl_error("lua_pred: name must be a string") end
    
    for _, f in ipairs(current_module.lua_pred_funcs) do
        if f.name == func_name then
            dsl_error("Lua pred function already declared: " .. func_name)
        end
    end
    
    table.insert(current_module.lua_pred_funcs, {
        name = func_name,
        hash = hash_module.fnv1a_32(func_name),
    })
    
    local found = false
    for _, n in ipairs(current_module.pred_funcs) do
        if n == func_name then found = true; break end
    end
    if not found then table.insert(current_module.pred_funcs, func_name) end
end

function _G.LUA_FUNCTIONS(tbl)
    if not current_module then dsl_error("No module started") end
    if tbl.oneshot then
        for _, name in ipairs(tbl.oneshot) do lua_oneshot(name) end
    end
    if tbl.main then
        for _, name in ipairs(tbl.main) do lua_main(name) end
    end
    if tbl.pred then
        for _, name in ipairs(tbl.pred) do lua_pred(name) end
    end
end
