...
static bool dispatch_pred(
    ...
    if (mod->pred_fns[func_idx]) {
        inst->current_func_hash = mod->def->pred_hashes[func_idx];
...
static s_expr_result_t dispatch_main(
    ...
    // Set hash once — covers INIT, TICK, and TERMINATE phases below
    inst->current_func_hash = mod->def->main_hashes[func_idx];
