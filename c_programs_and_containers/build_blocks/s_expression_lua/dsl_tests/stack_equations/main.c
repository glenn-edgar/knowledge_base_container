...
#include "stack_equations_bin_32.h"

#include <time.h>


extern void stack_equations_register_all(s_expr_module_t* module);

static void register_user_functions(s_engine_handle_t* engine) {
    stack_equations_register_all(&engine->module);
}
...
int main(int argc, char* argv[]) {
    ...
    bool result = s_engine_load_from_rom(
        &engine,
        &alloc,
        stack_equations_module_bin_32,
        STACK_EQUATIONS_MODULE_BIN_32_SIZE,
        debug_callback,
        1,
        user_fns
    );
