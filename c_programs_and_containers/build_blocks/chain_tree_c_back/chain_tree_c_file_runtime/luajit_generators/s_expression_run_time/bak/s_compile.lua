...
local function main()
    ...
    -- Try to load s_cfl_helpers.lua if it exists
    local cfl_helpers = find_file("s_cfl_helpers.lua", search_paths)
    if cfl_helpers then
        local ok, err = pcall(dofile, cfl_helpers)
        if not ok then
            io.stderr:write("Warning: Error loading s_cfl_helpers.lua: " .. tostring(err) .. "\n")
        end
    end
