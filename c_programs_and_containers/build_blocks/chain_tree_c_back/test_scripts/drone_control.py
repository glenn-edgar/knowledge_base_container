...
class DroneControl(ControlledNodes):
    def __init__(self, ctb, h_file: str):
        ...
        fly_arc_container = {}
        fly_arc_container["request_port"] = self.make_port(self.h_file, 2, "fly_arc_request")
        fly_arc_container["response_port"] = self.make_port(self.h_file, 3, "fly_arc_response")
        fly_arc_container["api_name"] = "drone_control_fly_arc"
        self.command_container["fly_arc"] = fly_arc_container
        
        fly_up_container = {}
        fly_up_container["request_port"] = self.make_port(self.h_file, 4, "fly_up_request")
        fly_up_container["response_port"] = self.make_port(self.h_file, 5, "fly_up_response")
        fly_up_container["api_name"] = "drone_control_fly_up"
        self.command_container["fly_up"] = fly_up_container
        
        fly_down_container = {}
        fly_down_container["request_port"] = self.make_port(self.h_file, 6, "fly_down_request")
        fly_down_container["response_port"] = self.make_port(self.h_file, 7, "fly_down_response")
    ...
    def fly_arc_client(self, distance: float, final_altitude: float, final_speed: float, heading: float, finalize_fn: str, finalize_data: dict):
        ...
        self.client_controlled_node(fly_arc_container["api_name"], finalize_fn, monitor_data, 
