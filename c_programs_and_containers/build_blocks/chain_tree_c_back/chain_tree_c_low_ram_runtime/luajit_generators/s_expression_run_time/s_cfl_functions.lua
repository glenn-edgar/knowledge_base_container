...
-- Copy ROM constants to RAM blackboard fields
-- Note: io_call versions survive reset (run only on first init)
--       o_call versions run every time
--============================================================================

--============================================================================
-- CFL_COPY_CONST - Copy a ROM constant to a specific blackboard field
-- Usage: cfl_copy_const("pid_gains", "aggressive_pid")
--============================================================================

function cfl_copy_const(field_path, const_name)
    if type(field_path) ~= "string" then
        dsl_error("cfl_copy_const() field_path must be a string")
    end
    if type(const_name) ~= "string" then
        dsl_error("cfl_copy_const() const_name must be a string")
    end
...
-- Init version: survives reset, runs only once
function cfl_init_copy_const(field_path, const_name)
    if type(field_path) ~= "string" then
        dsl_error("cfl_init_copy_const() field_path must be a string")
    end
    if type(const_name) ~= "string" then
        dsl_error("cfl_init_copy_const() const_name must be a string")
    end
    
    local c = io_call("CFL_COPY_CONST")
        if field_path:find("%.") then
            nested_field_ref(field_path)
        else
            field_ref(field_path)
...
--============================================================================
-- CFL_COPY_CONST_FULL - Copy a ROM constant to the entire blackboard
-- Usage: cfl_copy_const_full("default_config")
--============================================================================

function cfl_copy_const_full(const_name)
    if type(const_name) ~= "string" then
        dsl_error("cfl_copy_const_full() const_name must be a string")
    end
...
-- Init version: survives reset, runs only once
function cfl_init_copy_const_full(const_name)
    if type(const_name) ~= "string" then
        dsl_error("cfl_init_copy_const_full() const_name must be a string")
    end
    
    local c = io_call("CFL_COPY_CONST_FULL")
        const_ref(const_name)
    end_call(c)
end
