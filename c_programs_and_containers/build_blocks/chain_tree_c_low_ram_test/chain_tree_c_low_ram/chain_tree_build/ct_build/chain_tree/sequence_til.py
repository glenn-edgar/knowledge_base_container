...
class SequenceTil(ColumnFlow):
    ...
    def add_exception_recovery_link(self, except_node_id:str,link_id:str, disable_columns:list,enable_columns:list):
        except_node = self.ctb.yaml_data[except_node_id]
    ...
    def asm_turn_heartbeat_on(self,parent_node_name:str,time_out:int):
        ...
        return self.asm_one_shot_handler("CFL_TURN_HEARTBEAT_ON",{"node_id":node_id,"time_out":time_out})
    
    def asm_turn_heartbeat_off(self,parent_node_name:str):
        node_id = self.ctb.get_node_index(parent_node_name)
        return self.asm_one_shot_handler("CFL_TURN_HEARTBEAT_OFF",{"node_id":node_id})
    
    def asm_heartbeat_event(self,parent_node_name:str):

        node_id = self.ctb.get_node_index(parent_node_name)
        return self.asm_one_shot_handler("CFL_HEARTBEAT_EVENT",{"node_id":node_id})
