 ...
 */
 ...
 typedef enum {
     ...
     CFL_EVENT_TYPE_NULL        = 6   /**< Data is null */
 } cfl_event_type_t;
  ...
  * 
  * Dual-priority queue with independent sizing and statistics tracking.
  * Size: ~32 bytes on 32-bit systems, ~44 bytes on 64-bit systems.
  */
 typedef struct CFL_EVENT_QUEUE_T {
     ...
     uint16_t            max_total_depth;    /**< Peak total events across both queues */
     uint16_t            max_high_depth;     /**< Peak high priority queue depth */
  ...
  * 
  * Allocates control structure and event arrays from the specified permanent allocator.
  ...
  * @param perm               Permanent allocator to use
  * @return Pointer to queue control structure, or throws exception on failure
  * 
  ...
  * 
  * @note Throws exception if queue_control is NULL, priority invalid, or event_type invalid
  * 
  * Example:
  * @code
  *   // Send pointer event
  ...
  * 
  * Examines the next event that would be returned by cfl_pop_event()
  ...
  * @note Useful for examining event before deciding to process it
  * @note Throws exception if queue_control or event_data is NULL
  * 
  * Example:
  * @code
  *   CFL_EVENT_DATA_T event;
  *   if (cfl_peek_event(q, &event)) {
  *       // Check if we should process this event
  *       if (event.node_id == my_node_id) {
  *           // Now actually remove and process it
  *           cfl_pop_event(q, &event);
  *           process_event(&event);
  *       }
  *   }
  * @endcode
