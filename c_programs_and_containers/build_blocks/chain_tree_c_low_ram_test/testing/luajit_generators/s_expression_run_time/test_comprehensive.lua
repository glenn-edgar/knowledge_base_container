...
-- Updated for DSL v2.5 with SE_ result codes
...
local tree9 = start_tree("dispatch_test")
    -- Note: dispatch(key, prefix) - key is first argument
...
local tree10 = start_tree("debug_test")
    -- Note: dbg(message, prefix) - message is first argument
...
-- Generate C header
local header = module:to_c_header("test")
local f = io.open("test_module.h", "w")
f:write(header)
f:close()
print("\nGenerated test_module.h")
