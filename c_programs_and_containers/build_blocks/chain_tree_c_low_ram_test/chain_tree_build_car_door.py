...
def insert_bitmask_column(ct):
    ...
    ct.asm_one_shot_handler(one_shot_fn="INITIALIZE_MOTOR_BITMASK",one_shot_data={})
...
def insert_emergency_stop(ct):
    ...
    ct.asm_one_shot_handler(one_shot_fn="SEND_SERIAL_MESSAGE_EMERGENCY_STOP",one_shot_data={})
    ct.asm_wait_for_bitmask(bitmask_event_list=["EMERGENCY_STOP_BIT"],reset_flag=False,timeout=5000)
    ct.asm_halt()
    ct.end_column(column_name=emergency_stop_inner_column)
    ct.define_join_link(parent_node_name=emergency_stop_inner_column)
    ct.asm_clear_bitmask(event_list=["EMERGENCY_STOP_BIT"])
...
def insert_callibration(ct):
    ...
    ct.asm_wait("CALLIBRATION_POSITION_TIMEOUT",wait_fn_data=None, reset_flag = False, timeout=3000, error_fn = "CALLIBRATION_TOO_LONG", error_data = None )
    ct.asm_terminate()
    ...
    ct.define_column_link(main_function_name="STORE_EPROM_DATA", initialization_function_name="STORE_EPROM_DATA_INIT",
                          aux_function_name="CFL_NULL",termination_function_name="CFL_NULL",node_data={})
    ct.asm_clear_bitmask(event_list=["CALLIBRATION_BIT"])
...
def insert_open_door(ct):
    ...
    ct.asm_wait("OPEN_DOOR_VERIFY",wait_fn_data=None, reset_flag = False, timeout=3000, error_fn = "DOOR_OPEN_TOO_LONG", error_data = None )
    ct.asm_terminate()
    ct.asm_one_shot_handler("SET_MOTOR_DIRECTION",one_shot_data={"direction":"STOP"})
    ct.asm_one_shot_handler("SEND_SERIAL_MESSAGE_CLOSE_DOOR_COMPLETE",one_shot_data={})
    ct.asm_clear_bitmask(event_list=["CLOSE_DOOR_BIT"])
    ct.asm_halt()
    ...
    ct.asm_one_shot_handler("LOG_FINAL_POSITION",one_shot_data={})
    ct.asm_clear_bitmask(event_list=["OPEN_DOOR_BIT"])
    ct.asm_halt()
...
def insert_close_door(ct):
    ...
    ct.asm_wait("CLOSE_DOOR_VERIFY",wait_fn_data=None, reset_flag = False, timeout=3000, error_fn = "DOOR_CLOSE_TOO_LONG", error_data = None )
    ct.asm_terminate()
    ct.asm_one_shot_handler("SET_MOTOR_DIRECTION",one_shot_data={"direction":"STOP"})
    ct.asm_one_shot_handler("SEND_SERIAL_MESSAGE_CLOSE_DOOR_COMPLETE",one_shot_data={})
    ct.asm_clear_bitmask(event_list=["CLOSE_DOOR_BIT"])
...
def insert_motor(ct):
    motor_column = ct.define_column(column_name="motor_column", column_data=None,auto_start=True)
...
def insert_current_monitor(ct):
    ...
    ct.asm_wait_time(time_delay=.01)
...
def insert_status_monitor(ct):
    ...
    ct.define_column_link(main_function_name="STATUS_MONITOR_MAIN", initialization_function_name="STATUS_MONITOR_INIT", aux_function_name="CFL_NULL",
                           termination_function_name="CFL_NULL", node_data={})
    ct.asm_wait_for_event(event_id="STATUS_REQUEST_EVENT",event_count=1,reset_flag=True,timeout=100,error_fn="CFL_NULL",error_data={})
    ct.asm_reset()
