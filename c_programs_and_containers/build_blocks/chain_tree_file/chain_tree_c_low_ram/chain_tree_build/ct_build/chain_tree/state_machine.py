...
class StateMachine(ColumnFlow):
    ...
    def initialize_state_machine_stack(self):
        self.sm_stack = []
        self.sm_name_dict = {}
    ...
    def define_state(self,state_name:str,column_data:dict=None):
        ...
        state_node_id = self.ctb.get_node_index(state_node)
    ...
    def change_state(self,sm_node_id:str,new_state:str,sync_event_id:str=None):
        ...
        node_id = self.ctb.get_node_index(sm_node_id)
        node_date = { "node_id": node_id, "new_state": new_state, "sync_event_id": sync_event_id}
    ...
    def end_state_machine(self,state_node:str,sm_name:str):
        ...
        ref_data = self.ctb.config_data[state_node]
    ...
    def __initialize_state_links(self, state_node :str, sm_name :str, state_names :list[str],initial_state:str):
        ...
        self.ctb.config_data[state_node] = ref_data
        
        
        
        
    def _add_state_link(self, state:str,node_id :int):
        if len(self.sm_stack) == 0:
            raise ValueError("State machine not defined")
        if not isinstance(node_id, int):
            raise TypeError("node_id must be an integer")
        ref_node = self.ctb.ltree_stack[-2]
        ref_data = self.ctb.config_data[ref_node]
        if "state_links" not in ref_data["label_dict"]:
            raise ValueError("state machine not found")
        ...
        ref_data["label_dict"]["state_links"].append(node_id)
        self.ctb.config_data[ref_node] = ref_data