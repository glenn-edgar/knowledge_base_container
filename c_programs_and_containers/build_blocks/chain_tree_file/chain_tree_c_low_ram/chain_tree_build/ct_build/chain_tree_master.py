...
from  .chain_tree.chain_tree_resolver import ChainTreeJsonResolver
from .chain_tree.chain_tree_json import ChainTreeJson
...
class ChainTreeMaster(BasicCfLinks, WaitCfLinks, VerifyCfLinks, StateMachine, SequenceTil, DataFlow, ExceptionHandler):
    ...
    def __init__(self, json_file: Path):
        self.ctb = ChainTreeJson(json_file)
    ...
    def check_and_generate_json(self, resolve: bool = True):
        """
        Validate and generate JSON configuration.
        
        Args:
            resolve: If True, also generate resolved JSON for Zig/C runtime
        """
        ...
        if resolve:
            # Generate resolved version with indices
            resolved_file = self.json_file.with_suffix('.resolved.json')
            resolver = ChainTreeJsonResolver(self.json_file)
    ...
    def get_all_virtual_functions(self):
        ...
        main_functions = self.ctb.config_data[f"{base_path}.main_functions"]["node_dict"].keys()
        one_shot_functions = self.ctb.config_data[f"{base_path}.one_shot_functions"]["node_dict"].keys()
        boolean_functions = self.ctb.config_data[f"{base_path}.boolean_functions"]["node_dict"].keys()
    ...
    def dump_kb_functions(self):
        """Dump all function mappings as nodes in the json structure."""
