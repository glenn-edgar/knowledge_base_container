...
function ModuleGenerator:to_bin()
    ...
    local function emit_u8(v)
        table.insert(out, bit.band(v or 0, 0xFF))
    end
    ...
    for _, tree_name in ipairs(self.tree_order) do
        ...
        -- Emit nodes (14 bytes each) - WITH NIL GUARDS
        for _, n in ipairs(nodes) do
            emit_u8(n.type or 0)
            emit_u8(n.child_count or 0)
            emit_u16(n.node_index or 0)
            emit_u16(n.first_child or 0xFFFF)
            emit_u16(n.next_sibling or 0xFFFF)
            emit_u16(n.fn_index or 0)
            emit_u16(n.param_offset or 0)
            emit_u8(n.param_count or 0)
    ...
    for _, tree_name in ipairs(self.tree_order) do
        ...
        for i, n in ipairs(nodes) do
            print(string.format(
                ...
                i - 1, n.type or 0, n.child_count or 0, n.first_child or 0xFFFF, 
                n.next_sibling or 0xFFFF, n.fn_index or 0, n.param_offset or 0, 
                n.param_count or 0
            ))
        end
    end
