 ...
 */
 ...
 static void rdr_on_connect(struct mosquitto *mosq, void *userdata, int rc)
 {
     ...
     /*
      * libmosquitto v1 on_connect doesn't expose session_present directly.
      * With clean_session=false and a fixed client_id, the broker *will*
      * resume the session and deliver queued QoS1+ messages automatically
      * after CONNACK.  We track this via the config flag; on the second
      * connect with the same client_id the session is assumed present.
      *
      * NOTE: mosquitto >= 2.0 exposes session_present in on_connect_v5;
      * for v1 API we rely on convention.
      */
