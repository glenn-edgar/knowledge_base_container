...
class KVStoreWriter:
    ...
    def _on_publish(self, client, userdata, mid, reason_code, properties):
        ...
        if self.use_mqttv5:
            ...
            # For v3.1.1, if we get here the publish completed
            success = True

        with self._results_lock:
            self._publish_results[mid] = (success, f"reason_code={reason_code}")

        # Signal the event for this message ID (if already registered)
        with self._events_lock:
            ev = self._publish_events.get(mid)
            if ev is not None:
                ev.set()
    ...
    def connect(self, timeout: float = 5.0) -> bool:
        ...
        try:
            # Connect to broker
            if self.use_mqttv5:
                # Set clean_start at connect time for v5
    ...
    def disconnect(self) -> None:
        ...
        try:
            # Prefer disconnect() first, then stop the loop (avoids races)
    ...
    def _register_event_and_arm_mid(self, mid: int, ev: threading.Event) -> None:
        """
        Internal helper to register a publish event for a given mid and
        immediately set it if the publish result already arrived (race-safe).
        """
        with self._events_lock:
            self._publish_events[mid] = ev
        # Race fix: on_publish might have already populated results
        with self._results_lock:
    ...
    def write_single(
        ...
        # Create and arm event for this publish (race-safe)
        publish_event = threading.Event()
        self._register_event_and_arm_mid(result.mid, publish_event)
        ...
        if success:
            s = value_str if isinstance(value_str, str) else (value_str.decode(errors="replace") if isinstance(value_str, bytes) else str(value_str))
            print(f"[Written] {topic} => {s[:50]}{'...' if len(s) > 50 else ''}")
        ...
        if parallel:
            ...
            for topic, value in kv_pairs.items():
                ...
                if result.rc != mqtt.MQTT_ERR_SUCCESS:
                    ...
                    self._register_event_and_arm_mid(result.mid, event)
            ...
            per_message_timeout = max(0.1, timeout / len(kv_pairs))
            for topic, value in kv_pairs.items():
    ...
    def delete_single(self, topic: str, timeout: float = 2.0) -> bool:
        ...
        # Create and arm event (race-safe)
        delete_event = threading.Event()
        self._register_event_and_arm_mid(result.mid, delete_event)
