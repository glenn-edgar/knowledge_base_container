...
class KB_Status_Table:
    ...
    def get_status_data(self, path):
        ...
        try:
            ...
            # Extract data and path from result (sqlite3.Row object)
    ...
    def set_status_data(self, path: str, data: Dict[str, Any], 
        ...
        # SQLite UPSERT query
        # Note: SQLite doesn't have xmax, so we'll check if rowid changed to detect insert vs update
        # First check if the path exists
        upsert_query = f"""
            INSERT INTO {self.base_table} (path, data)
            VALUES (?, ?)
            ON CONFLICT (path)
        ...
        while attempt <= retry_count:
            try:
                # Check if path exists before operation (to determine insert vs update)
                check_query = f"SELECT path FROM {self.base_table} WHERE path = ?"
                self.kb_search.cursor.execute(check_query, (path,))
                existed_before = self.kb_search.cursor.fetchone() is not None
                ...
                if result is not None:
                    ...
                    operation = "updated" if existed_before else "inserted"
    ...
    def set_multiple_status_data(self, path_data_pairs: Union[Dict[str, Dict], List[Tuple[str, Dict]]], 
        ...
        while attempt <= retry_count:
            try:
                ...
                # Get existing paths to determine insert vs update
                all_paths = [path for path, _ in json_pairs]
                placeholders = ','.join(['?'] * len(all_paths))
                check_query = f"SELECT path FROM {self.base_table} WHERE path IN ({placeholders})"
                self.kb_search.cursor.execute(check_query, tuple(all_paths))
                existing_paths = {row['path'] if isinstance(row, sqlite3.Row) else row[0] 
                                for row in self.kb_search.cursor.fetchall()}
                
                # Use batch operations within a transaction
                for path, json_data in json_pairs:
                    existed_before = path in existing_paths
