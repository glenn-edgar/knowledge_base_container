...
class KB_Data_Structures:
    ...
    def __init__(self, db_file, database):
        """
        Initialize KB_Data_Structures with SQLite database.
        
        Args:
            db_file (str): Path to the SQLite database file
            database (str): Database/schema name identifier
        """
        self.query_support = KB_Search(db_file, database)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python kb_data_structures.py <database_file.db>")
        print("Example: python kb_data_structures.py knowledge_base.db")
        sys.exit(1)
