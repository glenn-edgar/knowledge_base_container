...
class Construct_Job_Table:
    """
    This class is designed to construct a job table with header
    ...
    def _setup_schema(self):
        ...
        # Index on path for ltree operations
        create_path_index = f"""
        ...
        create_schedule_index = f"""
            CREATE INDEX IF NOT EXISTS idx_{self.table_name}_schedule_at ON {self.table_name} (schedule_at)
        """
        self.cursor.execute(create_schedule_index)
        
        # Index on is_active for filtering active jobs
        create_active_index = f"""
            CREATE INDEX IF NOT EXISTS idx_{self.table_name}_is_active ON {self.table_name} (is_active)
        """
        self.cursor.execute(create_active_index)
        
        # Index on valid for filtering valid jobs
        create_valid_index = f"""
            CREATE INDEX IF NOT EXISTS idx_{self.table_name}_valid ON {self.table_name} (valid)
        """
        self.cursor.execute(create_valid_index)
        
        # Composite index on is_active and schedule_at for common job queries
        create_composite_index = f"""
            CREATE INDEX IF NOT EXISTS idx_{self.table_name}_active_schedule 
            ON {self.table_name} (is_active, schedule_at)
        """
        self.cursor.execute(create_composite_index)
        
        # Index on started_at for tracking job execution times
        create_started_index = f"""
            CREATE INDEX IF NOT EXISTS idx_{self.table_name}_started_at ON {self.table_name} (started_at)
        """
        self.cursor.execute(create_started_index)
        
        # Index on completed_at for tracking job completion times
        create_completed_index = f"""
            CREATE INDEX IF NOT EXISTS idx_{self.table_name}_completed_at ON {self.table_name} (completed_at)
        """
    ...
    def add_job_field(self, job_key, job_length, description):
        ...
        Args:
            job_key (str): The key/name of the job field
            job_length (int): The length of the job
            description (str): The description of the job queue
            
        Raises:
            TypeError: If job_key is not a string or job_length is not an integer
        """
    ...
    def _manage_job_table(self, specified_job_paths, specified_job_length):
        """
        Manages the number of records in job_table to match specified job lengths for each path.
        Removes older records first if necessary and adds new ones with NULL for JSON data.
        
        Args:
            specified_job_paths (list): Array of valid paths
        ...
        for i in range(len(specified_job_paths)):
            ...
            count_query = f"SELECT COUNT(*) FROM {self.table_name} WHERE path = ?"
            ...
            if diff < 0:
                ...
                insert_query = f"""
                    INSERT INTO {self.table_name} (path, data)
                    VALUES (?, ?)
                """
    ...
    def _remove_invalid_job_fields(self, invalid_job_paths, chunk_size=500):
        ...
        Args:
            invalid_job_paths (list): Array of paths that should be removed from the database
    ...
    def check_installation(self):     
        ...
        - Add entries to job_table for paths in knowledge_base that don't exist in job_table
        """
        
        query = f"SELECT DISTINCT path FROM {self.table_name}"
        self.cursor.execute(query)
        ...
        # Get specified job paths from knowledge base
        query = f"""
        ...
        specified_job_length = []
        
        # Parse JSON properties to get job_length
        for row in specified_job_data:
            props_json = row[3]
            if props_json:
                props = json.loads(props_json)
                specified_job_length.append(props.get('job_length', 0))
            else:
                specified_job_length.append(0)
