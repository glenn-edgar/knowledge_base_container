...
/* =====================================================================
 * Register application-specific boolean functions with binary image loader.
 ...
 * ===================================================================== */
 ...
 void cfl_register_user_boolean_functions(cfl_image_loader_t *img)
 {
     ...
     #define REG_APP_BOOL(name_str, sym) do { \
         rc = cfl_image_register_boolean(img, (name_str), (sym)); \
         if (rc < 0) { fprintf(stderr, "  WARN: app boolean not found: %s\n", name_str); missing++; } \
     } while(0)
