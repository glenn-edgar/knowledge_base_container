...
# Avro/streaming/drone functions are registered in C via cfl.register_c_user_functions()
...
# avro_verify_const_packet_main and avro_verify_packet_main are in C
...
# Streaming/drone boolean functions are in C via cfl.register_c_user_functions()
...
def main():
    ...
    # Register C avro/streaming/drone functions (compiled into micropython binary)
    cfl.register_c_user_functions()

    # Register Python one-shot functions
    for name, func in [
