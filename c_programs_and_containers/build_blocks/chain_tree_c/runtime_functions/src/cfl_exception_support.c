...
static unsigned find_parent_node_exception(cfl_runtime_handle_t *runtime_handle, uint16_t node_id, bool terminate_flag){
    ...
    while (!found_flag) {
        ...
        if( node->main_function_index == runtime_handle->main_function_data->main_function_ids[CFL_FUNCTION_ID_JSON_CONTROLLED_NODE_MAIN]) {

            return search_node_id;
        }
        if( node->main_function_index == runtime_handle->main_function_data->main_function_ids[CFL_FUNCTION_ID_CBOR_CONTROLLED_NODE_MAIN]) {
...
void cfl_raise_json_exception_event(cfl_runtime_handle_t *runtime_handle, uint16_t node_id, uint16_t parent_node_id )
{
  ...
  cfl_send_json_record_event(runtime_handle->event_queue, CFL_EVENT_PRIORITY_HIGH, parent_node_id, CFL_RAISE_EXCEPTION_EVENT, node_id);
