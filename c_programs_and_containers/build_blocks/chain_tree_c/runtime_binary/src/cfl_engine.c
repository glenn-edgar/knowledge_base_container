 ...
 */
 ...
 static CT_ReturnCode cfl_execute_node(void *user_handle, unsigned int node_id,
     ...
     const cfl_chaintree_node_t *node = &handle->flash_handle->nodes[node_id];
     ...
     const cfl_main_function_t main_function =
         handle->flash_handle->main_functions[node->main_function_index];
     const cfl_one_shot_function_t one_shot_function =
         handle->flash_handle->one_shot_functions[node->init_function_index];
     const cfl_boolean_function_t boolean_function =
 ...
 static unsigned int cfl_get_forward_enabled_links(void *user_handle, unsigned int node_id,
     ...
     uint16_t link_count = (node->link_count & CFL_LINK_COUNT_MASK);
 ...
 static void cfl_disable_node(cfl_runtime_handle_t *handle, unsigned node_index) {
     ...
     const cfl_chaintree_node_t *node = &handle->flash_handle->nodes[node_index];
 ...
 void cfl_terminate_node_tree(cfl_runtime_handle_t *handle, unsigned node_id) {
     ...
     if ((node->link_count & CFL_LINK_COUNT_MASK) == 0) {
         cfl_disable_node(handle, node_id);
         return;
     }
 ...
 static CT_ReturnCode cfl_find_try_mark_node(void *handle, unsigned node_index,
     ...
     cfl_runtime_handle_t *runtime_handle = (cfl_runtime_handle_t *)handle;
     const cfl_chaintree_node_t *node = &runtime_handle->flash_handle->nodes[node_index];
