...
void cfl_s_expression_node_init_one_shot_fn(cfl_runtime_handle_t* handle, uint16_t node_index) {
    ...
    *tree_inst_ptr = tree_inst;
    ...
    s_expr_tree_instance_t** tree_inst_data = (s_expr_tree_instance_t**)cfl_smart_arena_alloc(handle, node_index, sizeof(s_expr_tree_instance_t*));
    if (!tree_inst_data) {
        EXCEPTION("failed to allocate tree instance data");
        return;
    }
    
    *tree_inst_data = tree_inst;
