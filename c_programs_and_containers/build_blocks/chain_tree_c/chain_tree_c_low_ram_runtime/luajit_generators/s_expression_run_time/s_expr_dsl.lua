...
local PARAM_OPCODES = {
    ...
    RESULT    = 0x0C,
    STR_IDX   = 0x0D,
    CONST_REF = 0x0E,   -- Constant structure reference
}

local TYPE_FLAGS = {
    SURVIVES_RESET = 0x40,  -- bit 6: io_call
...
-- Helper: resolve_field_path - Walk nested path and return field info
local function resolve_field_path(record, path)
    local parts = {}
    for part in string.gmatch(path, "[^%.]+") do
        table.insert(parts, part)
    end
    
    local current_record = record
    ...
    for i, part in ipairs(parts) do
        ...
        if i < #parts then
            -- Not the final part, must be embedded record
            if not field.is_embedded then
                return nil
            end
            current_record = _module.records[field.type_name]
            if not current_record then
                return nil
            end
        else
            -- Final part - return info
            return {
...
function CONST(name, record_type)
    check_in_module("CONST")
    
    if _module.current_record then
        dsl_error("CONST() cannot be inside RECORD()")
    end
    
    if _module.current_const then
        dsl_error("CONST() cannot be nested, missing END_CONST()?")
    end
    
    if _module.current_tree then
        dsl_error("CONST() cannot be inside a tree")
    end
    
    if not _module.records[record_type] then
        dsl_error("CONST() unknown record type: " .. record_type)
    end
    
    if _module.constants[name] then
        dsl_error("CONST() duplicate name: " .. name)
    end
    
    _module.current_const = name
    _module.constants[name] = {
        name = name,
    ...
    debug_print("CONST:", name, "(" .. record_type .. ")")
end

function VALUE(field_path, value)
    check_in_module("VALUE")
    
    if not _module.current_const then
        dsl_error("VALUE() must be inside CONST()")
    end
    
    local const_def = _module.constants[_module.current_const]
    local record = _module.records[const_def.record_type]
    
    -- Validate field path exists and get type info
    local field_info = resolve_field_path(record, field_path)
    if not field_info then
        dsl_error("VALUE() invalid field path: " .. field_path)
    end
    
    -- Store value with type info
    const_def.values[field_path] = {
        value = value,
        field_info = field_info,
    }
    table.insert(const_def.value_order, field_path)
    
    debug_print("  VALUE:", field_path, "=", tostring(value))
end

function END_CONST()
    check_in_module("END_CONST")
    
    if not _module.current_const then
        dsl_error("END_CONST() without matching CONST()")
    end
    
    table.insert(_module.constant_order, _module.current_const)
    
    debug_print("END_CONST:", _module.current_const)
    
    _module.current_const = nil
end
...
function const_ref(name)
    check_in_tree("const_ref")
    check_not_in_bit_block("const_ref")
    
    if not _module.constants[name] then
        dsl_error("const_ref() unknown constant: " .. name)
    end
    
    local const_def = _module.constants[name]
    local record = _module.records[const_def.record_type]
    
    -- Find index of this constant
    local const_index = 0
    for i, cname in ipairs(_module.constant_order) do
        if cname == name then
            const_index = i - 1
            break
        end
    ...
    emit_param({
        type = PARAM_OPCODES.CONST_REF,
        index_to_pointer = 0,
        node_index = 0,
        const_name = name,
        const_index = const_index,
        record_type = const_def.record_type,
        const_size = record.total_size,
        value_type = "const_ref",
    })
end
...
function start_module(name)
    ...
    _module = {
        ...
        constant_order = {},         -- Order of constant definitions
        current_const = nil,         -- Currently defining constant
        pools = {},
        slots = {},
        pool_counter = 0,
        current_tree = nil,
        ...
        pt_m_call_funcs = {},
        ptr_field_refs = {},
        string_table = {},           -- Array of unique strings
        string_table_map = {},       -- String content -> index lookup
    }
...
function end_module(name)
    ...
    if _module.current_const then
        dsl_error("end_module() while still in const '" .. _module.current_const .. "' - missing END_CONST()?")
    end
    ...
    debug_print("end_module:", name, 
        ...
        "constants=" .. #_module.constant_order)
...
--============================================================================
-- RECORDS HEADER GENERATION (standalone, no DSL internals)
--============================================================================

function ModuleGenerator:to_c_records_header(base_name)
    local lines = {}
    local mod = self.module
    local guard = string.upper(base_name) .. "_RECORDS_H"
    
    table.insert(lines, "// ============================================================================")
    table.insert(lines, "// " .. base_name .. "_records.h")
    table.insert(lines, "// Generated by ChainTree S-Expression DSL v3.0")
    table.insert(lines, "// Record structure definitions - safe to include in user code")
...
function ModuleGenerator:param_to_c(param)
    ...
    elseif param.value_type == "const_ref" then
        struct = string.format("{ .type = %s, .const_index = %d, .const_size = %d }",
            type_hex, param.const_index, param.const_size)
        comment = param.const_name
...
--============================================================================
-- CONST INITIALIZER GENERATION
--============================================================================

function ModuleGenerator:generate_const_initializer(const_def, record, indent)
    local lines = {}
    local mod = self.module
    
    -- Build a tree of values from flat paths
    local value_tree = {}
    
    for _, path in ipairs(const_def.value_order) do
        local val_info = const_def.values[path]
        local parts = {}
        for part in string.gmatch(path, "[^%.]+") do
            table.insert(parts, part)
        end
        ...
        -- Insert into tree
        local current = value_tree
        for i, part in ipairs(parts) do
            if i == #parts then
                current[part] = val_info
            else
    ...
    local function format_value(val_info, value)
        local c_type = val_info.field_info.c_type
        local type_name = val_info.field_info.type_name
        
        if c_type == "float" then
            return string.format("%.6ff", value)
        elseif c_type == "double" then
            return string.format("%.10f", value)
        elseif c_type == "bool" then
            return value and "true" or "false"
        elseif type_name == "uint32" or type_name == "uint16" or type_name == "uint8" then
            if type(value) == "number" and value >= 0x1000 then
                return string.format("0x%08X", value)
            else
                return tostring(value)
            end
        elseif type_name == "int32" or type_name == "int16" or type_name == "int8" then
            return tostring(value)
        else
            return tostring(value)
        end
    ...
    -- Generate initializer recursively
    local function generate_level(node, rec, ind)
        local result = {}
        
        for _, field_name in ipairs(rec.field_order) do
            local field = rec.fields[field_name]
            local node_val = node[field_name]
            
            if node_val then
                if type(node_val) == "table" and not node_val.value then
                    -- Nested struct (node_val is a subtree, not a value entry)
                    local embedded_record = mod.records[field.type_name]
                    if embedded_record then
                        table.insert(result, ind .. "." .. field_name .. " = {")
                        local nested = generate_level(node_val, embedded_record, ind .. "    ")
                    ...
                    -- Simple value (node_val has .value and .field_info)
                    local formatted = format_value(node_val, node_val.value)
                    table.insert(result, string.format("%s.%s = %s,", ind, field_name, formatted))
                end
            end
        end
    ...
    return generate_level(value_tree, record, indent)
end
...
function ModuleGenerator:to_c_header(base_name)
    ...
    -- Include records header if records exist
    if #mod.record_order > 0 then
        table.insert(lines, "#include \"" .. base_name .. "_records.h\"")
    end
    table.insert(lines, "")
    
    -- Function hash tables (NO forward declarations or record structures here)
    table.insert(lines, "// ============================================================================")
    ...
    -- Constant structures (ROM)
    if #mod.constant_order > 0 then
        table.insert(lines, "// ============================================================================")
        table.insert(lines, "// CONSTANT STRUCTURES (ROM)")
        table.insert(lines, "// ============================================================================")
        table.insert(lines, "")
        
        local base_name_lower = base_name:lower():gsub("[^%w_]", "_")
        
        for _, const_name in ipairs(mod.constant_order) do
            local const_def = mod.constants[const_name]
            local record = mod.records[const_def.record_type]
            
            table.insert(lines, string.format("static const %s_t %s_%s = {",
                const_def.record_type, base_name_lower, const_name))
            
            -- Generate initializer values
            local init_lines = self:generate_const_initializer(const_def, record, "    ")
            for _, line in ipairs(init_lines) do
        ...
        -- Generate constants pointer array
        table.insert(lines, "// Constants pointer table")
        table.insert(lines, "static const void* const " .. base_name .. "_constants[] = {")
        for i, const_name in ipairs(mod.constant_order) do
            local comma = (i < #mod.constant_order) and "," or ""
            table.insert(lines, string.format("    &%s_%s%s  // index %d",
                base_name_lower, const_name, comma, i - 1))
        end
        table.insert(lines, "};")
        table.insert(lines, "")
    end
    ...
    -- Add constants table
    if #mod.constant_order > 0 then
        table.insert(lines, "    .constants = " .. base_name .. "_constants,")
        table.insert(lines, "    .const_count = " .. #mod.constant_order .. ",")
    else
        table.insert(lines, "    .constants = NULL,")
        table.insert(lines, "    .const_count = 0,")
    end
...
function ModuleGenerator:dump()
    ...
    if #mod.constant_order > 0 then
        ...
        for _, name in ipairs(mod.constant_order) do
            local const_def = mod.constants[name]
            table.insert(lines, "  " .. name .. " (" .. const_def.record_type .. ")")
            for _, path in ipairs(const_def.value_order) do
                local val_info = const_def.values[path]
                table.insert(lines, "    " .. path .. " = " .. tostring(val_info.value))
            end
        end
        table.insert(lines, "")
    end
