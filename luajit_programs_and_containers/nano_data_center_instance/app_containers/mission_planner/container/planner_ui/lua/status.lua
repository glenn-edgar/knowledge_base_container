...
function M.summary_key(site)
  return site .. ".action_server.summary"
end

function M.status_key(site, robot_id)
  return site .. ".action_server." .. robot_id .. ".status"
end

function M.result_key(site, robot_id)
  return site .. ".action_server." .. robot_id .. ".result"
end
...
--
-- opts test shim: { ks_lib, cjson, site, nats_url, worker_id }.
function M.list_missions(opts)
  ...
  opts.site = site; opts.nats_url = nats_url
  ...
  local get_ok, val = pcall(function()
    return ks:get(M.summary_key(site))
  end)
...
function M.get_mission(robot_id, opts)
  ...
  local status, serr = read_json(M.status_key(site, robot_id))
  if serr then return nil, serr end
  local result, rerr = read_json(M.result_key(site, robot_id))
