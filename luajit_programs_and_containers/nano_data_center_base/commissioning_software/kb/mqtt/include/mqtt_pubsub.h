 ...
 * callback (mosquitto thread) → LuaJIT drains them with poll() from
 * the main thread. No re-subscribe per read.
 *
 * Designed for low-latency bidirectional robot communication:
 *   - ESP32 / Pico 2 W publish to stream topic
 *   - Bridge subscribes once, polls buffered messages
 *   - Bridge publishes commands to rpc topic
 */
