...
return {
  ...
  agent_exceptions = {
    ...
    node_control = {
      ...
      { name = "container_respawned",
        type = "docker", instance = "node_control", priority = 3,
        description = "Reconcile loop detected a missing assigned container and respawned it",
        response_procedure = "Inspect container logs for the preceding crash. Repeat respawns within a short window indicate a chronic fault; consider shelving the container via maintenance lease." },
      { name = "container_hung",
        type = "docker", instance = "node_control", priority = 2,
        description = "Watchdog probe detected an assigned container is running but unresponsive; docker restart issued",
        response_procedure = "Inspect container logs for the hang (deadlock, exhausted pool, blocked cosocket). Repeated hangs within a short window indicate a chronic fault; escalate." },
    },
  },
