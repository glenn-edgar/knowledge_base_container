...
# pg_connector, ndc_paths) into _staged_lib/ so the analyzer processes can
# require them. These are added after luajit_base was built, so they're not
# yet in the base image layer; staging here keeps the container self-contained
...
rm -rf "$STAGE"
mkdir -p "$STAGE"
for f in kb_log.lua kb_rule.lua kb_exception.lua posix_time.lua pg_connector.lua ndc_paths.lua; do
    src="$DCS_RT/$f"
