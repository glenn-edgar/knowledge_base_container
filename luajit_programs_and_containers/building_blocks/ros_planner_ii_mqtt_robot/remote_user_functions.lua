...
-- === recharge ===
M.one_shot.WKR_RECHARGE_INIT = function(h) h.blackboard.exec_start = true; h.blackboard.exec_active = false end
M.main.WKR_RECHARGE_MAIN = function(h, bf, n, eid)
    if eid ~= defs.CFL_TIMER_EVENT then return defs.CFL_CONTINUE end
    local bb = h.blackboard
    if bb.exec_start then bb.exec_start = false; bb.exec_active = true; bb.ticks_remaining = action_durations[cmd_packets.TYPE_RECHARGE] end
    if bb.exec_active then bb.ticks_remaining = bb.ticks_remaining - 1; if bb.ticks_remaining <= 0 then bb.exec_active = false; return defs.CFL_DISABLE end end
    return defs.CFL_CONTINUE
end
