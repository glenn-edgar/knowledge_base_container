--[[
    test_mqtt_cbor.lua -- End-to-end: planner → MQTT (CBOR) → robot.

    Same 18-action route as test_mqtt_direct.lua but with CBOR wire format.
    Robot encodes stream responses as CBOR, hub decodes before processing.
    Hub encodes RPC commands as CBOR, robot decodes before executing.

    Two processes (started by run_tests.sh):
      1. This test (planner side, MQTT direct)
      2. remote_mqtt_ct.lua (MQTT robot, VMRT_WIRE_FORMAT=cbor)
]]
...
print(string.format("Robot: %s, MQTT: %s:%d, NATS: %s, wire: cbor\n",
...
-- Pre-set wire format to CBOR so poll() decodes inbound messages
...
print("Hub runtime initialized (MQTT/CBOR transport).")
