 ...
 *
 * Flat maps only: {"key": value, ...} where values are int, float,
 * string, bool, or null. No external dependencies beyond libc.
 */
...
int cbor_from_json(const char *json, uint8_t *out, int max_out) {
    ...
    while (*p && *p != '}') {
        ...
        if (*p == '"') {
            ...
            /* Number: try integer first, float if has dot/e */
            char *end;
            int is_float = 0;
            const char *scan = p;
            if (*scan == '-') scan++;
            ...
            if (is_float) {
                ...
                if (d == floor(d) && fabs(d) < 4294967296.0 && !is_float) {
                    goto encode_int;
                }
...
int cbor_to_json(const uint8_t *cbor, int cbor_len, char *out, int max_out) {
    int cpos = 0;  /* CBOR position */
    int jpos = 0;  /* JSON position */

    int major;
    uint64_t val;

    /* Top-level must be a map */
    int n = dec_head(cbor + cpos, cbor_len - cpos, &major, &val);
    if (n < 0 || major != CBOR_MAP) return -1;
    cpos += n;
    int map_count = (int)val;
    ...
    for (int i = 0; i < map_count; i++) {
        ...
        if (n < 0 || major != CBOR_TSTR) return -1;
        cpos += n;
        int key_len = (int)val;
        if (cpos + key_len > cbor_len) return -1;

        /* Write key as JSON string */
        if (jpos + key_len + 3 >= max_out) return -1;
        out[jpos++] = '"';
        memcpy(out + jpos, cbor + cpos, key_len);
        jpos += key_len;
        out[jpos++] = '"';
        out[jpos++] = ':';
        cpos += key_len;
        ...
        if (first == CBOR_TRUE) {
            ...
            /* Format: integer-valued doubles as integers */
