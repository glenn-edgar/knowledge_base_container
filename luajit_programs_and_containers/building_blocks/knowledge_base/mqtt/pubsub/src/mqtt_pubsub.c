/**
 * mqtt_pubsub.c - JSON-pubsub 2.0 over MQTT implementation
 *
 ...
 */
 ...
 /* ================================================================== */
 /*  pubsub Server — callbacks                                             */
 /* ================================================================== */
 
 static void srv_on_connect(struct mosquitto *mosq, void *userdata, int rc)
 {
     UNUSED(mosq);
     pubsub_server_t *srv = (pubsub_server_t *)userdata;
 
     pthread_mutex_lock(&srv->lock);
     srv->connected = (rc == 0);
     printf("[pubsub_server] on_connect rc=%d (%s)\n", rc,
     ...
     if (srv->connected) {
         ...
         if (res != MOSQ_ERR_SUCCESS)
             fprintf(stderr, "[pubsub_server] subscribe error: %s\n", mosquitto_strerror(res));
     }
 ...
 static void srv_on_disconnect(struct mosquitto *mosq, void *userdata, int rc)
 {
     ...
     pthread_mutex_lock(&srv->lock);
     srv->connected = false;
     printf("[pubsub_server] on_disconnect rc=%d\n", rc);
     pthread_cond_signal(&srv->connect_cond);
     pthread_mutex_unlock(&srv->lock);
 }
                              ...
                              int mid, int qos_count, const int *granted_qos)
 {
     ...
     pubsub_server_t *srv = (pubsub_server_t *)userdata;
     printf("[pubsub_server] on_subscribe mid=%d\n", mid);
     pthread_mutex_lock(&srv->lock);
     srv->subscribed = true;
 ...
 static void *srv_process_thread(void *arg)
 {
     ...
     cJSON_AddStringToObject(resp, "jsonpubsub", "2.0");
 
     /* Validate JSON-pubsub shape */
     cJSON *ver = cJSON_GetObjectItem(req, "jsonpubsub");
     ...
     if (!ver || !cJSON_IsString(ver) || strcmp(ver->valuestring, "2.0") != 0 ||
         ...
         cJSON_AddNumberToObject(err, "code", JSONPUBSUB_INVALID_REQUEST);
         ...
         /* Look up method */
         pubsub_method_entry_t *entry = NULL;
         pthread_mutex_lock(&srv->lock);
         for (pubsub_method_entry_t *e = srv->methods; e; e = e->next) {
         ...
         if (!entry) {
             cJSON *err = cJSON_CreateObject();
             cJSON_AddNumberToObject(err, "code", JSONPUBSUB_METHOD_NOT_FOUND);
             char msg[128];
             ...
             if (result) {
                 ...
                 cJSON *err = cJSON_CreateObject();
                 cJSON_AddNumberToObject(err, "code", JSONPUBSUB_INTERNAL_ERROR);
                            ...
                            const struct mosquitto_message *msg)
 {
     ...
     /*
      * Expected topic: pubsub/{service}/request/{caller_client_id}
     ...
     if (!req) {
         fprintf(stderr, "[pubsub_server] failed to parse JSON request\n");
         return;
     }
     ...
     if (pthread_create(&tid, &attr, srv_process_thread, ctx) != 0) {
         fprintf(stderr, "[pubsub_server] pthread_create failed\n");
         cJSON_Delete(req);
         free(ctx->caller_id);
         free(ctx);
     }
 ...
 int pubsub_server_init(pubsub_server_t *srv, const mqtt_pubsub_config_t *cfg)
 {
     ...
     srv->client_id = cfg->client_id ? strdup(cfg->client_id)
                                     : auto_client_id("pubsub_server");
 
     const char *svc = cfg->service_name ? cfg->service_name : "pubsub_service";
     srv->request_topic       = aprintf("pubsub/%s/request/+", svc);
     srv->response_topic_base = aprintf("pubsub/%s/response", svc);
     ...
     if (!srv->mosq) {
         fprintf(stderr, "[pubsub_server] mosquitto_new: %s\n", strerror(errno));
         return -1;
     }
 ...
 {
     pubsub_method_entry_t *e = calloc(1, sizeof(*e));
     if (!e) return;
     e->name     = strdup(name);
     e->fn       = fn;
     e->userdata = userdata;
     ...
     printf("[pubsub_server] registered method: %s\n", name);
 }
 ...
 int pubsub_server_start(pubsub_server_t *srv, bool wait_for_suback, int sub_timeout_ms)
 {
     ...
     if (rc != MOSQ_ERR_SUCCESS) {
         fprintf(stderr, "[pubsub_server] connect: %s\n", mosquitto_strerror(rc));
         return -1;
     }
 
     rc = mosquitto_loop_start(srv->mosq);
     if (rc != MOSQ_ERR_SUCCESS) {
         fprintf(stderr, "[pubsub_server] loop_start: %s\n", mosquitto_strerror(rc));
         return -1;
     }
     ...
     pthread_mutex_unlock(&srv->lock);
     if (!ok) { fprintf(stderr, "[pubsub_server] connect timeout\n"); return -1; }
     ...
     if (wait_for_suback) {
         ...
         if (!srv->subscribed)
             fprintf(stderr, "[pubsub_server] warning: SUBACK not received yet\n");
         pthread_mutex_unlock(&srv->lock);
     }
 
     printf("[pubsub_server] started (service=%s)\n", srv->cfg.service_name);
     return 0;
 }
 ...
 {
     /* Free method list */
     pubsub_method_entry_t *e = srv->methods;
     while (e) {
         pubsub_method_entry_t *next = e->next;
         free(e->name);
         free(e);
         e = next;
     }
 ...
 /* ================================================================== */
 /*  pubsub Client — callbacks                                             */
 /* ================================================================== */
 
 static void cli_on_connect(struct mosquitto *mosq, void *userdata, int rc)
 {
     UNUSED(mosq);
     pubsub_client_t *cli = (pubsub_client_t *)userdata;
 
     pthread_mutex_lock(&cli->lock);
     cli->connected = (rc == 0);
     printf("[pubsub_client] on_connect rc=%d (%s)\n", rc,
            rc == 0 ? "success" : mosquitto_connack_string(rc));
     ...
     if (cli->connected) {
         ...
         if (res != MOSQ_ERR_SUCCESS)
             fprintf(stderr, "[pubsub_client] subscribe error: %s\n", mosquitto_strerror(res));
     }
 ...
 static void cli_on_disconnect(struct mosquitto *mosq, void *userdata, int rc)
 {
     ...
     pthread_mutex_lock(&cli->lock);
     cli->connected = false;
     printf("[pubsub_client] on_disconnect rc=%d\n", rc);
     pthread_cond_signal(&cli->connect_cond);
     pthread_mutex_unlock(&cli->lock);
 }
                              ...
                              int mid, int qos_count, const int *granted_qos)
 {
     UNUSED(mosq); UNUSED(qos_count); UNUSED(granted_qos);
     pubsub_client_t *cli = (pubsub_client_t *)userdata;
     printf("[pubsub_client] on_subscribe mid=%d\n", mid);
     pthread_mutex_lock(&cli->lock);
     cli->subscribed = true;
                            ...
                            const struct mosquitto_message *msg)
 {
     ...
     pthread_mutex_lock(&cli->lock);
     for (pubsub_pending_t *p = cli->pending_head; p; p = p->next) {
                     ...
                     float default_timeout_s)
 {
     ...
     cli->client_id = cfg->client_id ? strdup(cfg->client_id)
                                     : auto_client_id("pubsub_client");
 
     const char *svc = cfg->service_name ? cfg->service_name : "pubsub_service";
     cli->request_topic  = aprintf("pubsub/%s/request/%s", svc, cli->client_id);
     cli->response_topic = aprintf("pubsub/%s/response/%s", svc, cli->client_id);
     ...
     if (!cli->mosq) {
         fprintf(stderr, "[pubsub_client] mosquitto_new: %s\n", strerror(errno));
         return -1;
     }
 ...
 int pubsub_client_connect(pubsub_client_t *cli, int timeout_ms)
 {
     ...
     if (rc != MOSQ_ERR_SUCCESS) {
         fprintf(stderr, "[pubsub_client] connect: %s\n", mosquitto_strerror(rc));
         return -1;
     }
 
     rc = mosquitto_loop_start(cli->mosq);
     if (rc != MOSQ_ERR_SUCCESS) {
         fprintf(stderr, "[pubsub_client] loop_start: %s\n", mosquitto_strerror(rc));
         return -1;
     }
     ...
     if (!ok) {
         fprintf(stderr, "[pubsub_client] connect timeout\n");
     ...
     if (!cli->subscribed)
         fprintf(stderr, "[pubsub_client] warning: SUBACK not received yet\n");
     pthread_mutex_unlock(&cli->lock);
 ...
 {
     printf("[pubsub_client] disconnecting...\n");
 ...
 void pubsub_client_destroy(pubsub_client_t *cli)
 {
     ...
     pubsub_pending_t *p = cli->pending_head;
     while (p) {
         pubsub_pending_t *next = p->next;
         free(p->id);
                     ...
                     cJSON **out_error)
 {
     ...
     if (!cli->connected) {
         fprintf(stderr, "[pubsub_client] call: not connected\n");
         return -1;
     }
     ...
     /* Build JSON-pubsub request */
     cJSON *req = cJSON_CreateObject();
     cJSON_AddStringToObject(req, "jsonpubsub", "2.0");
     ...
     pubsub_pending_t *pend = calloc(1, sizeof(*pend));
     pend->id   = req_id;
     pend->done = false;
     ...
     if (rc != MOSQ_ERR_SUCCESS) {
         fprintf(stderr, "[pubsub_client] publish failed: %s\n", mosquitto_strerror(rc));
         /* Remove pending */
         pthread_mutex_lock(&cli->lock);
         pubsub_pending_t **pp = &cli->pending_head;
         while (*pp) {
     ...
     if (!pend->done) {
         fprintf(stderr, "[pubsub_client] call '%s' timed out after %.1fs\n", method, tout);
         free(pend->id);
         pthread_cond_destroy(&pend->cond);
         free(pend);
         return -1;
 ...
 static void *async_thread(void *arg)
 {
     ...
     int rc = pubsub_client_call(ctx->cli, ctx->method, ctx->params,
                             ...
                             void *cb_userdata)
 {
     ...
     /* Generate the id that pubsub_client_call will use (peek at next counter) */
     pthread_mutex_lock(&cli->lock);
