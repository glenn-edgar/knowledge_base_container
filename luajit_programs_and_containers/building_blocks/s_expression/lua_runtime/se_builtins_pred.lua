...
-- Increments field each call; returns true when it reaches limit, then resets.
-- params[1] = field_ref,  params[2] = limit,  params[3] = reset_value (opt, default 0)
-- ----------------------------------------------------------------------------
M.se_field_increment_and_test = function(inst, node)
    local v     = field_get(inst, node, 1) or 0
    local limit = param_int(node, 2)
    v = v + 1
    se_runtime.field_set(inst, node, 1, v)
    if v >= limit then
        local reset_val = (node.params[3] and param_int(node, 3)) or 0
        se_runtime.field_set(inst, node, 1, reset_val)
        return true
    end
    return false
end
...
-- Increments node_state.user_data each call; returns true at limit, then resets.
-- params[1] = limit
-- ----------------------------------------------------------------------------
M.se_state_increment_and_test = function(inst, node)
    local ns    = get_ns(inst, node.node_index)
    local limit = param_int(node, 1)
    ns.user_data = (ns.user_data or 0) + 1
