    ...
    if tree.record_name then
        ...
        if rec and rec.fields then
            for field_name, field_def in pairs(rec.fields) do
                inst.blackboard[field_name] = field_def.default or 0
            end
        end
    end
...
-- Reset a child: ACTIVE | EVER_INIT, clear state/user_data (preserve user flags)
...
local function field_get(inst, node, i)
    return inst.blackboard[node.params[i].value]
end
