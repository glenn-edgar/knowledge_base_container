...
-- se_log_int: print field as integer
-- params[1] = field_ref
M.se_log_int = function(inst, node)
    local v = field_get(inst, node, 1)
    print("[SE_LOG_INT] " .. tostring(v))
end

-- se_log_float: print field as float
-- params[1] = field_ref
M.se_log_float = function(inst, node)
    local v = field_get(inst, node, 1)
    print(string.format("[SE_LOG_FLOAT] %g", tonumber(v) or 0))
end
...
-- se_set_field: write integer constant to field
-- params[1] = field_ref,  params[2] = int value
M.se_set_field = function(inst, node)
    field_set(inst, node, 1, param_int(node, 2))
end
...
-- se_set_external_field: in Lua, same as se_set_field (blackboard is the external)
-- params[1] = field_ref,  params[2] = int value
...
M.se_queue_event = function(inst, node)
    ...
    se_runtime.event_push(inst, tt, eid, nil)
end
...
-- se_push_stack: push params[1] integer value onto inst.stack
M.se_push_stack = function(inst, node)
    if inst.stack then
        inst.stack.top = (inst.stack.top or 0) + 1
        inst.stack[inst.stack.top] = param_int(node, 1)
    end
end

-- ----------------------------------------------------------------------------
-- Function dictionary loading (no-op in LuaJIT; dispatch is direct table lookup)
-- ----------------------------------------------------------------------------
M.se_load_function_dict = function(inst, node) end
M.se_load_function      = function(inst, node) end
