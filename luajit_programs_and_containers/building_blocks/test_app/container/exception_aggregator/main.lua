#!/usr/bin/env luajit
-- exception_aggregator/main.lua -- SHELL. Registration-test only.
-- Prints one heartbeat line per second to stderr; no real work done.
...
io.stderr:write(string.format(
    "exception_aggregator: started name=%s container=%s\n", app, cont))
io.stderr:flush()
...
while true do
    i = i + 1
    io.stderr:write(string.format("exception_aggregator[%s]: tick %d\n", app, i))
    io.stderr:flush()
