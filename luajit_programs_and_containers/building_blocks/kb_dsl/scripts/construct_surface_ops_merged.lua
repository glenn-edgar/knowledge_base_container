--[[
    construct_surface_ops_merged.lua — Merged KB for surface_ops domain

    Builds a SQLite KB that merges:
      1. System infrastructure from the master Postgres KB
         (CPU, containers, services — same ltree namespace)
      2. Full domain content (boards, robot classes, virtual nodes, bitmasks,
         status fields, stream fields — same as construct_surface_ops.lua)

    The result is a single SQLite DB where the planner can query both
    infrastructure endpoints and domain-specific data using the same
    ltree paths as the master Postgres.

    Usage:
      cd kb_dsl/scripts
      luajit construct_surface_ops_merged.lua

    Output:
      ../sqlite_dbs/surface_ops_merged.db
]]

-----------------------------------------------------------------------
-- Step 1: Run master_build to populate Postgres, then extract system
--         infrastructure into the SQLite DB
-----------------------------------------------------------------------

-- Package path for Postgres construct_kb (master build needs it)
local pg_kb_path = "../../knowledge_base/postgres/construct_kb/?.lua"
package.path = pg_kb_path .. ";" .. package.path

-- Load config
local site_config = require("site_config")

-- Connect to Postgres (upload_flag=true: read-only, don't recreate tables)
local PG_Construct_KB = require("construct_kb")
local pg_kb = PG_Construct_KB.new(
  os.getenv("PG_HOST") or "localhost",
  tonumber(os.getenv("PG_PORT")) or 5432,
  os.getenv("PG_DBNAME") or "knowledge_base",
  os.getenv("PG_USER") or "gedgar",
  os.getenv("POSTGRES_PASSWORD") or "",
  "knowledge_base",  -- table_name
  true)              -- upload_flag = true (skip table creation)

local json = require("dkjson")

-----------------------------------------------------------------------
-- Step 2: Create the SQLite DB using construct_data_tables (full API)
-----------------------------------------------------------------------

-- Swap to SQLite modules
local sqlite_dir = "../../knowledge_base/sqlite3/construct_kb/"
local saved_loaded = {}
for _, name in ipairs({"construct_kb", "knowledge_base_manager"}) do
...
-- Restore Postgres modules (we still need pg_kb for queries)
for name, mod in pairs(saved_loaded) do
  package.loaded[name] = mod
end
package.path = saved_path

local DB_FILE = "../sqlite_dbs/surface_ops_merged.db"
local DATABASE = "knowledge_base"
local SITE = "moonbase.alpha.services.robot_control.surface_ops"

-- Remove old DB
os.remove(DB_FILE)

local ltree_path = sqlite_dir .. "ltree"
local kb = CDT.new(DB_FILE, DATABASE, ltree_path)

-----------------------------------------------------------------------
-- Step 3: Insert system infrastructure from Postgres
-----------------------------------------------------------------------
print("=== Merged Surface Ops KB Build ===")
print("\n--- System infrastructure from Postgres ---")

kb:add_kb("system", "Physical topology (from master)")
kb:select_kb("system")

local domain_cfg = nil
for _, d in ipairs(site_config.domains) do
  if d.name == "surface_ops" then domain_cfg = d; break end
end
assert(domain_cfg, "surface_ops domain not found in site_config")

local cpu_path = "system.site." .. site_config.site_name .. ".cpu." .. domain_cfg.cpu
local container_path = cpu_path .. ".container." .. domain_cfg.container

--- Helper: query Postgres and insert into SQLite
local function pg_to_sqlite(kb_name, sql)
  local rows = pg_kb:_query(sql)
  for _, row in ipairs(rows) do
    local props = json.decode(row.properties) or {}
    local data  = json.decode(row.data) or {}
    kb.kb._kb:add_node(kb_name, row.label, row.name, props, data, row.path)
    print("  " .. row.path)
  end
  return rows
end

-- Site root + CPU + master role
pg_to_sqlite("system", string.format(
  "SELECT label, name, properties::text, data::text, path::text FROM %s "..
  "WHERE knowledge_base = 'system' AND path::text = 'system.site.%s' ORDER BY path",
  pg_kb.table_name, site_config.site_name))

pg_to_sqlite("system", string.format(
  "SELECT label, name, properties::text, data::text, path::text FROM %s "..
  "WHERE knowledge_base = 'system' AND path::text = '%s' ORDER BY path",
  pg_kb.table_name, cpu_path))

pg_to_sqlite("system", string.format(
  "SELECT label, name, properties::text, data::text, path::text FROM %s "..
  "WHERE knowledge_base = 'system' AND path::text = '%s.role.master' ORDER BY path",
  pg_kb.table_name, cpu_path))

-- All infrastructure containers + their services
pg_to_sqlite("system", string.format(
  "SELECT label, name, properties::text, data::text, path::text FROM %s "..
  "WHERE knowledge_base = 'system' AND path::text LIKE '%s.container.%%' "..
  "AND properties::text LIKE '%%infrastructure%%' ORDER BY path",
  pg_kb.table_name, cpu_path))

pg_to_sqlite("system", string.format(
  "SELECT label, name, properties::text, data::text, path::text FROM %s "..
  "WHERE knowledge_base = 'system' AND label = 'service' "..
  "AND path::text LIKE '%s.container.%%' "..
  "AND path::text NOT LIKE '%s.container.%s%%' ORDER BY path",
  pg_kb.table_name, cpu_path, cpu_path, domain_cfg.container))

-- This domain's container + services
pg_to_sqlite("system", string.format(
  "SELECT label, name, properties::text, data::text, path::text FROM %s "..
  "WHERE knowledge_base = 'system' AND path::text LIKE '%s%%' ORDER BY path",
  pg_kb.table_name, container_path))

-----------------------------------------------------------------------
-- Step 4: Build domain content (same as construct_surface_ops.lua)
-----------------------------------------------------------------------
print("\n--- Domain content ---")
...
kb:add_header_node("robots", "rover_1", {}, {},
    ...
    kb:add_status_field("connection", {},
        "Connection info",
        {
            ...
            rpc_topic       = "moonbase.alpha.services.robot_control.surface_ops.robots.rover_1.rpc",
            stream_bus_topic = "moonbase.alpha.services.robot_control.surface_ops.robots.rover_1.stream_bus",
        })
...
kb:add_header_node("robots", "rover_2", {}, {},
    ...
    kb:add_status_field("connection", {},
        "Connection info",
        {
            ...
            robot_id        = "rover_2",
            nats_server     = "nats://127.0.0.1:4222",
            rpc_topic       = "moonbase.alpha.services.robot_control.surface_ops.robots.rover_2.rpc",
            stream_bus_topic = "moonbase.alpha.services.robot_control.surface_ops.robots.rover_2.stream_bus",
        })
...
kb:add_header_node("robots", "arm_1", {}, {},
    ...
    kb:add_status_field("connection", {},
        "Connection info",
        {
            ...
            robot_id        = "arm_1",
            nats_server     = "nats://127.0.0.1:4222",
            rpc_topic       = "moonbase.alpha.services.robot_control.surface_ops.robots.arm_1.rpc",
            stream_bus_topic = "moonbase.alpha.services.robot_control.surface_ops.robots.arm_1.stream_bus",
        })
...
-- Post-install: copy initial status data
...
print("\n=== Merged Surface Ops KB Built ===")
print("Database: " .. DB_FILE)
print("KBs: system (infrastructure) + " .. SITE .. " (domain)")
