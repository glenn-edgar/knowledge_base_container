...
-- =========================================================================
-- Test 4: Sequential s-engine link tests (thirty_two_test)
-- =========================================================================

local function thirty_two_test(ct, kb_name)
    ct:start_test(kb_name)
    ...
    local launch_column = ct:define_column("launch_column", nil, nil, nil, nil, nil, true)
        ...
        local node_id_a = ct:se_engine_link("chain_flow_dsl_tests",
            "s_expression_test_10", "se_link_10_ptr")
        ct:define_join_link(node_id_a)

        local node_id_b = ct:se_engine_link("chain_flow_dsl_tests",
            "s_expression_test_11", "se_link_11_ptr")
        ct:define_join_link(node_id_b)

        local node_id_c = ct:se_engine_link("chain_flow_dsl_tests",
            "s_expression_test_12", "se_link_12_ptr")
        ct:define_join_link(node_id_c)

        local node_id_d = ct:se_engine_link("chain_flow_dsl_tests",
            "s_expression_test_13", "se_link_13_ptr")
        ...
        local node_id_e = ct:se_engine_link("chain_flow_dsl_tests",
            "s_expression_test_14", "se_link_14_ptr", user_data)
        ct:define_join_link(node_id_e)
        ct:asm_log_message("s expression link node test 14 is not active")

        local node_id_f = ct:se_engine_link("chain_flow_dsl_tests",
            "s_expression_test_15", "se_link_15_ptr")
        ct:define_join_link(node_id_f)
        ct:asm_log_message("s expression link node test 15 is not active")

        local node_id_h = ct:se_engine_link("chain_flow_dsl_tests",
            "s_expression_test_16", "se_link_16_ptr", {},
            "TEST_39_SET_INIT_DATA")
...
local function add_header(yaml_file)
    ...
    ct:define_blackboard("se_test_2_state")
        ...
        ct:bb_field("se_link_10_ptr", "uint64", 0)
        ct:bb_field("se_link_11_ptr", "uint64", 0)
        ct:bb_field("se_link_12_ptr", "uint64", 0)
        ct:bb_field("se_link_13_ptr", "uint64", 0)
        ct:bb_field("se_link_14_ptr", "uint64", 0)
        ct:bb_field("se_link_15_ptr", "uint64", 0)
        ct:bb_field("se_link_16_ptr", "uint64", 0)
