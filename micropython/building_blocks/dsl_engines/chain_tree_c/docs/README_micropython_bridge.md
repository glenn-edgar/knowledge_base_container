...
Unlike the Lua 5.3 bridge (which uses a C main.c), the MicroPython bridge is a **compiled-in C module**. The entire test runs as a Python script:
...
│  │  "cfl" module: json, bb, arena, heap   │  │
│  │  Lifecycle: load, register, run        │  │
│  └───────────────┬────────────────────────┘  │
│                  │                           │
│  ┌───────────────▼────────────────────────┐  │
│  │        CFL C Runtime Engine            │  │
│  │  runtime_binary + runtime_functions    │  │
│  │  + s_expression (all compiled in)      │  │
│  └────────────────────────────────────────┘  │
└──────────────────────────────────────────────┘
```
...
flt = cfl.read_float(ptr, 4)     # Read float at ptr+4 (not available yet)
...
23/26 tests pass with pure Python user functions. The 3 remaining tests (19, 23, 24) require Avro packet C struct access — these would pass by compiling the Avro C files into the custom MicroPython binary.

All non-Avro tests — columns, forks, state machines, supervisors, exceptions, while loops, bitmask events, watchdogs, blackboard — work correctly through the Python bridge.
