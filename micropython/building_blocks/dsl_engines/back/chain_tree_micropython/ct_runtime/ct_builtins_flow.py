...
def cfl_sequence(inst, node, event_id, event_data):
    ...
    # All children complete — sequence done
...
def cfl_for_loop(inst, node, event_id, event_data):
    ...
    if event_id == rt.CT_EVENT_INIT:
        ...
        if nd and len(nd) >= 2 and nd[0] == "number_of_iterations":
            ns["user_data"] = int(nd[1])
        elif nd:
            ns["user_data"] = int(nd[0])
        else:
            ns["user_data"] = 0
        return rt.CT_CONTINUE
...
# ==========================================================================
# CFL_COLUMN_MAIN — DFS walker column
#
# Delegates child traversal to walk_column_dfs which visits active
# children in order and stops at the first CT_HALT (halt-stops-siblings).
# This matches the C runtime's walker semantics for CFL_COLUMN_MAIN.
# ==========================================================================
def cfl_column(inst, node, event_id, event_data):
    if event_id == rt.CT_EVENT_INIT:
        for i in range(len(node[rt.N_CHILDREN])):
            rt.child_reset(inst, node, i)
        return rt.CT_CONTINUE
...
builtins = {
    ...
    "CFL_LOCAL_ARENA_MAIN": cfl_column,
