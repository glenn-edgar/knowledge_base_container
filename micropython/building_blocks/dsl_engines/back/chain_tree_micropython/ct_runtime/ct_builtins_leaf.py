...
def cfl_disable(inst, node, event_id, event_data):
    if event_id == rt.CT_EVENT_INIT:
        # One-shot init functions are now dispatched centrally by invoke_main
        return rt.CT_CONTINUE
    if event_id == rt.CT_EVENT_TERMINATE:
        return rt.CT_CONTINUE
    # In C, cfl_disable_main_fn just returns CFL_DISABLE.
    # All one-shot behavior (logging, state changes, events) is handled
    # by _dispatch_init_fn during initialization.
    return rt.CT_DISABLE
...
def _find_sm_by_ltree(inst, ltree_path):
    ...
    if "_sm_ref_counter" not in inst:
