--[[
  ...
  Node tuple format (matches ct_runtime.py):
    (func_name, func_index, node_index, guard_index, node_data, children[, init_fn])
...
function MicroPythonCodeGenerator:_emit_node(lines, ltree_name, indent_level)
    ...
    -- Resolve ltree string references to node indices
    if nd and type(nd) == "table" then
        local filtered = {}
        for k, v in pairs(nd) do
            if k ~= "auto_start" then
                -- Resolve ltree path strings to node indices
                if type(v) == "string" and self.node_builder.ltree_to_final_index[v] then
                    filtered[k] = self.node_builder.ltree_to_final_index[v]
                else
                    filtered[k] = v
                end
            end
    ...
    -- Init function name for one-shot dispatch (index 6 in node tuple)
    local init_fn = functions.init or "CFL_NULL"
    ...
    -- Append init_fn as tuple element 6 when non-null
    local init_suffix = ""
    if init_fn ~= "CFL_NULL" then
        init_suffix = ", " .. py_str(init_fn)
    end

    if #real_children == 0 then
        -- Leaf node - single line
        lines[#lines + 1] = string.format('%s(%s, %d, %d, %d, %s, ()%s),',
            indent, py_str(main_fn), main_idx, node_index, guard_idx, nd_str, init_suffix)
    else
        ...
        lines[#lines + 1] = indent .. "    ),"
        if init_fn ~= "CFL_NULL" then
            lines[#lines + 1] = indent .. "    " .. py_str(init_fn) .. ","
        end
        lines[#lines + 1] = indent .. "),"
    end
end
