...
int main(int argc, char *argv[]) {
    ...
    // Default to the marked test (index 3) if no parameter is provided
    int test_index = 3; 
    
    // If a command line parameter is provided, parse it as the test index
    if (argc > 1) {
   ...
   /* ---- Create runtime (same as .h/.c path from here) ---- */
    ...
    // Run the requested or default test
