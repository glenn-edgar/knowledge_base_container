...
local function serialize_module_micropython(module_data)
    ...
    -- Build func_name -> index maps for pre-computing func_index
    local oneshot_idx = {}
    for i, name in ipairs(mod.oneshot_funcs) do oneshot_idx[name] = i - 1 end
    local main_idx = {}
    for i, name in ipairs(mod.main_funcs) do main_idx[name] = i - 1 end
    local pred_idx = {}
    for i, name in ipairs(mod.pred_funcs) do pred_idx[name] = i - 1 end

    local function get_func_index(node)
        local ct = node.call_type
        if ct == "o_call" or ct == "io_call" then
            return oneshot_idx[node.func_name] or 0
        elseif ct == "m_call" or ct == "pt_m_call" then
            return main_idx[node.func_name] or 0
        elseif ct == "p_call" or ct == "p_call_composite" then
            return pred_idx[node.func_name] or 0
        end
        return 0
    end

    -- DFS counter for node_index assignment
    local dfs_counter = {0}

    -- Recursive node serializer returning lines
    -- Node tuple: (func_name, func_hash, func_index, node_index, call_type,
    --              order, param_count, pointer_index, params, children)
    local function ser_node_tuple(node, indent)
        local ind  = string.rep("    ", indent)
        local ind2 = string.rep("    ", indent + 1)
        local parts = {}

        -- Assign DFS pre-order node_index
        local node_index = dfs_counter[1]
        dfs_counter[1] = dfs_counter[1] + 1
        ...
        table.insert(parts, ind2 .. string.format("%d,  # N_FUNC_INDEX", get_func_index(node)))
        table.insert(parts, ind2 .. string.format("%d,  # N_NODE_INDEX", node_index))
    ...
    table.insert(lines, "N_FUNC_INDEX    = const(2)")
    table.insert(lines, "N_NODE_INDEX    = const(3)")
    table.insert(lines, "N_CALL_TYPE     = const(4)")
    table.insert(lines, "N_ORDER         = const(5)")
    table.insert(lines, "N_PARAM_COUNT   = const(6)")
    table.insert(lines, "N_POINTER_INDEX = const(7)")
    table.insert(lines, "N_PARAMS        = const(8)")
    table.insert(lines, "N_CHILDREN      = const(9)")
    table.insert(lines, "")
    ...
    table.insert(lines, "")
    table.insert(lines, "# --- Tree tuple indices ---")
    table.insert(lines, "T_NAME           = const(0)")
    table.insert(lines, "T_NAME_HASH      = const(1)")
    table.insert(lines, "T_NODE_COUNT     = const(2)")
    table.insert(lines, "T_POINTER_COUNT  = const(3)")
    table.insert(lines, "T_RECORD_NAME    = const(4)")
    table.insert(lines, "T_DEFAULTS_INDEX = const(5)")
    table.insert(lines, "T_NODES          = const(6)")
    table.insert(lines, "")
    ...
    -- string_index as tuple of (name, index) pairs for ROM residency
    local skeys = {}
    for s, _ in pairs(mod.string_index) do table.insert(skeys, s) end
    table.sort(skeys)
    if #skeys > 0 then
    ...
    for ti, tname in ipairs(mod.tree_order) do
        local tree = mod.trees[tname]
        -- Reset DFS counter for each tree
        dfs_counter[1] = 0

        table.insert(lines, "    (")
        table.insert(lines, string.format("        %s,  # T_NAME", mpy_str(tree.name)))
        table.insert(lines, string.format("        0x%08X,  # T_NAME_HASH", tree.name_hash))
        table.insert(lines, string.format("        %d,  # T_NODE_COUNT", tree.node_count))
        table.insert(lines, string.format("        %d,  # T_POINTER_COUNT", tree.pointer_count or 0))
        table.insert(lines, string.format("        %s,  # T_RECORD_NAME",
            tree.record_name and mpy_str(tree.record_name) or "None"))
        table.insert(lines, string.format("        0x%04X,  # T_DEFAULTS_INDEX", tree.defaults_index or 0xFFFF))

        -- nodes as tuple of node-tuples
        table.insert(lines, "        (  # T_NODES")
        for i, node in ipairs(tree.nodes) do
            local nlines = ser_node_tuple(node, 3)
            for j, nl in ipairs(nlines) do
                table.insert(lines, nl)
            end
            lines[#lines] = lines[#lines] .. ","
    ...
    -- Records
    -- ====================================================================
    -- Records as tuple of record tuples: (name, name_hash, size, align, fields_tuple)
    -- Each field: (name, name_hash, type, offset, size, is_ptr, is_char, is_embed)
    table.insert(lines, "# --- Records ---")
    ...
    if #mod.record_order > 0 then
        ...
        for _, rname in ipairs(mod.record_order) do
            local rec = mod.records[rname]
            table.insert(lines, "    (")
            table.insert(lines, string.format("        %s,  # name", mpy_str(rec.name)))
            table.insert(lines, string.format("        0x%08X,  # name_hash", rec.name_hash))
            table.insert(lines, string.format("        %d,  # size", rec.size))
            table.insert(lines, string.format("        %d,  # align", rec.align))
            table.insert(lines, "        (  # fields")
            for i, f in ipairs(rec.fields) do
                table.insert(lines, string.format(
                    '            (%s, 0x%08X, %s, %d, %d, %s, %s, %s),',
    ...
    table.insert(lines, "# --- Events ---")
    if mod.events and #mod.events > 0 then
        -- events as tuple of (name, id) pairs
        table.insert(lines, "events = (")
        for _, evt in ipairs(mod.events) do
            table.insert(lines, string.format(
...
local function main()
    ...
    if not opts.header and not opts.user_header and not opts.registration and
       ...
       not opts.python_module and not opts.micropython_module and not opts.dump then
