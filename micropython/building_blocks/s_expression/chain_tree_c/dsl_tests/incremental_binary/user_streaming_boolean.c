...
#include "avro_packets/streaming_test_1.h"
...
void packet_generator_one_shot_fn(void *handle, unsigned node_index) {
    ...
    accelerometer_reading_wire_t *data = accelerometer_reading_packet_init(pkt, (uint8_t)node_index);
...
bool packet_sink_a_boolean_fn(void *handle, unsigned node_index, unsigned event_type, unsigned event_id, void *event_data) {
    ...
    const accelerometer_reading_wire_t *pkt = accelerometer_reading_packet_verify(packet);
    if (pkt == NULL) {
        EXCEPTION("packet_sink_a_boolean_fn: packet verify failed");
    }
    
    uint8_t source_node = packet->header.source_node;
...
bool packet_sink_b_boolean_fn(void *handle, unsigned node_index, unsigned event_type, unsigned event_id, void *event_data) {
    ...
    const accelerometer_reading_filtered_wire_t *pkt = accelerometer_reading_filtered_packet_verify(packet);
    if (pkt == NULL) {
...
typedef struct {
    ...
    accelerometer_reading_filtered_wire_t *out_data;
...
bool packet_transform_boolean_fn(void *handle, unsigned node_index, unsigned event_type, unsigned event_id, void *event_data) {
    ...
    if (event_id == CFL_INIT_EVENT) {
        if (node->user_data == NULL) {
            ...
            // Init packet header and get wire data pointer
            user->out_data = accelerometer_reading_filtered_packet_init(user->out_pkt, (uint8_t)node_index);
    ...
    const accelerometer_reading_packet_t *in_packet = (const accelerometer_reading_packet_t *)event_data;
    const accelerometer_reading_wire_t *in_pkt = accelerometer_reading_packet_verify(in_packet);
    if (in_pkt == NULL) {
...
bool packet_collector_sink_boolean_fn(void *handle, unsigned node_index, unsigned event_type, unsigned event_id, void *event_data) {
    ...
    for (unsigned i = 0; i < container->count; i++) {
        const accelerometer_reading_packet_t *packet = (const accelerometer_reading_packet_t *)container->packets[i];
        const accelerometer_reading_wire_t *pkt = accelerometer_reading_packet_verify(packet);
